...
Computer files don't often stay the same. They're updated with new information, new [metadata](/files/), things are moved around, [glitchy code is patched](/software-redesign/), [updates roll out to handle changes from *other* software](/software-redesign/), [databases](/database/) get cleaned up, and so on.

Naturally, across a [distributed system](/dist-sys/) (such as cloud synchronization), this can pose extremely complicated challenges. There are a few general ways to sync:
...
2. Create a [cryptographic hash](/encryption/) of the file's contents and track if anything changes, then update everything else to the latest and keep date/time-sorted backups of the changes.
...
Now, imagine combining the balancing of [peer-to-peer networking](/torrent/) into this. Instead of 1 central server holding *all* the information, you'd need to add another layer of complexity. This is where "blockchain" comes in.

To keep track of the changes, each [database record](/database/) is divided into manageable chunks of binary information called a "block". The records are strung together by cryptographic hashes on the beginning and end of the blocks:
...
This sequence of blocks is arranged into a [Merkle Tree](/data-structures/) and called the "chain of custody". Each hash is pulled from that block's code, so any changes would be inherently obvious.
...
However, because of [the way encryption works](/encryption/), a hash could be theoretically reproduced. In the above example, what stops a [hacker](/hacking/) from making a fraudulent Block 2 with the same Hash A and Hash B?
...
The nonces add onto other nonces, over and over, making data transfer mathematically arcane and horrifyingly inefficient. This entire setup is *very* [cybersecure](/cysec/), and creates very little room for hacking or error, but uses tons of electricity and processing power.
...
Bitcoin mining uses mathematical problems, and they're mined on a [logarithmic curve](/algebra/). That is, they become progressively more work-intensive to mine as the years go by. Only 21,000,000 Bitcoin can ever be "mined".

Crypto mining creates a severe workload on the drive and processor hardware, so many manufacturers have tried to limit crypto mining by making it work inefficiently or voiding the warranty, and [a few have actually used individuals' computers for their own gain without permission](/faang/).
...
1. In 2014, [a hacker](/hacking/) stole $50 million dollars of ETC from a startup that used Ethereum.
...
3. There was an attempt to make a soft [fork](/version-control/) (that would keep it all together), but it didn't work because of a [security vulnerability](/cysec/) with the consensus system, so the Ethereum Foundation had to create a hard fork called ETH.
...
If, for whatever reason, someone ever loses their [private keys](/encryption/) or they're stolen, they lose access to their blockchain. The decentralized network means there's nobody who can reset your password. In 2018, it was estimated that [about 20% of BTC is gone for good](https://web.archive.org/web/20180713203135/https://www.investopedia.com/news/20-all-btc-lost-unrecoverable-study-shows/).
...
However, it's hard to *clearly* define a comparison because most other monetary systems are spread across many other systems (e.g., banks, payment processors, US government, etc.). This problem *can* get better with optimization, but it takes [plenty of redesign](/software-maintenance/).
...
Irrespective of the risks and complexities, the decentralized nature of cryptocurrency and NFTs allow for many varieties of [exchange trading](https://notageni.us/investing/). For the most part, the industry is filled with an anti-authoritarian anarcho-capitalist culture that embraces the concept of a free market without government intervention, and is typically performed on a decentralized exchange (DEX).

What most of the crypto bros don't realize is that people are typically driven more by [use](https://gainedin.site/purpose/) than by ownership. People are generally happy to have banks hold their money if it's readily available for them later, and only resort to *not* using banks once they [distrust](https://gainedin.site/trust/) them.
...
In a lot of ways, the promotional angle for many blockchain implementations rings of an [MLM](https://notageni.us/mlm/) pitch: get rich quickly by getting in early, and pay someone else to figure everything out for you. It attracts the types of people willing to gamble while thinking they're making an investment, and adds in [the allure of technology](/trends/) to its offering.
...
Regulating cryptocurrency is complicated. [Accountants](https://notageni.us/accounting/) first determined crypto was best treated as a commodity, but it's a strange one that has complicated legal ramifications. For one, commodities *usually* have a geographic location. In practice, it's more like an [algorithmically](/algorithms/) generated [intellectual property](https://gainedin.site/ip/) than anything else.
...
There are [legal](https://notageni.us/legally-safe/) disputes over the concept of "code as law", where computer code is defined as the law itself (as opposed to the *spirit* of what the code was meant to do originally). "Code as law" would allow hackers to legally steal cryptocurrency if they can change the code of the blockchain.

Decentralized systems are a foreign concept because our human nature tends to direct our [power](https://gainedin.site/power/) into a pyramid, so it'll take quite some time for the chaos to settle about how to regulate crypto. However, crypto is *also* the platform for a *lot* of black market activity, so there's a lot of money and motivation by many people who want to subvert existing laws to keep those laws out of crypto.
...
And, since [investing](https://notageni.us/investing/) is regulated based on [predictions about the future](https://gainedin.site/imagination/), any whiff of regulatory change will dramatically change the value of a blockchain implementation. The philosophical definition of [what humanity deems as having value](https://gainedin.site/values/) is critically important to understand.

One inescapable reality about cryptocurrency, however, is that it *does* create a parallel tradable commodity that works enough like money, and people can trade with it if their government's currency becomes unstable. Of course, if a government were to oppress it enough, [fear](https://gainedin.site/fear/) would prevent it being anything *but* a black market activity.
...
- It can't provide services to the [underclass](https://gainedin.site/classes/) (e.g., healthcare, foster care, most social services).

## Privacy concerns

The [aspect of privacy](/cysec/) is also a concern with some blockchain implementations. In many respects, scanning and encoding every human eyeball (like with Worldcoin) or every fingerprint creates a tremendous risk to the natural anonymity we are all accustomed to. Thus, some activists like [Edward Snowden](https://web.archive.org/web/20211101005702/https://decrypt.co/84277/snowden-slams-sam-altman-worldcoin-eyeball-scan-for-crypto) are vehemently opposed to this type of implementation.

At the same time, its effectiveness only applies to the degree that it works. Very often, databases can be corrupted, and people can [hack](/hacking/) anything that can make them money (such as, for example, using fake [biometric data](/authentication/) if there's a cash reward for submitting your personal biological information).

While it's true that cryptocurrency is its own computerized accountant (instead of a bank), this comes at the risk that it's a dumb computer that has *no* intuition to [authenticate](/authentication/) you versus someone else with a photo of you and your password.

## Hacks/crimes

[Every typical hacking situation](/hacking/) applies with cryptocurrency, but often with less government pushback than money-based transactions. There are often bots that will "frontrun" transactions with minor changes to glean money on the side, and it's an even more brilliant tactic if they can stay undetected during the transfer.

The claims around using cryptocurrency is that it will fix corruption, but it can't. [Human nature](https://gainedin.site/humanity/) has moral failings that have *nothing* to do with money, since everyone has consented to *money* for thousands of years. Even within the first decade of cryptocurrency, bad actors have arisen.
...
One painful reality, though, is that almost anything that *could* be done with a blockchain implementation would work just as well without one. The only exception to this is illegal activity, though [not all of it is necessarily criminal](https://theologos.site/persecution/).
...
The [trend](https://gainedin.site/trends/) is still early enough that it's difficult to tell for sure. It may have a place as a high-risk [investment vehicle](https://notageni.us/investing/), but not much else right now.
...
At the same time, cryptocurrency can very effectively serve as the de facto currency standard if a government ever collapses into anarchy in a way that preserves its nation's [internet](/web-dev/).
