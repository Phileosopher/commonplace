...
"Optical mark recognition" (OMR) is still useful for multiple-choice [tests in schools](https://gainedin.site/pedagogy/) (informally called "scantrons"), but scanning in fixed information only has limited use compared to the conveniences of [a keyboard](/keyboard/).

## Scanning

A scanner ends up producing the same abstraction as a [camera](/camera/): a recorded image. However, the primary difference between the two is that a scanner will work line-by-line while a camera captures the image as instantaneously as possible.

The primary advantage of a scanner is that it can capture [text](/keyboard/) better, since it's taking proportionally more time to acquire the information. However, scanners are also cheaper than cameras because of that same reality.

In one sense, a flatbed scanner works a bit like the reverse of a [printer](/printer/): slowly move from one side to another, often with a light to illuminate the information, and a sensor captures the image and converts it into [encoded color](/graphics/) line-by-line.
...
- Portable scanner - a smaller and lower-quality scanner that can sometimes fit in a pocket, useful for gathering lots of [information](/database/) for review later.
- Multifunction scanner/printer/copier - a multi-tool [marketed](https://notageni.us/marketing/) as a [money-saving](https://adequate.life/money-4/) purchase that does nothing particularly well.

The primary [driver](/os/) technology that runs scanners is called "TWAIN", which is a reference to "never the twain shall meet" in light of how difficult connecting a scanner to a computer can be. That's not an exaggeration, and printer/scanner technology is irritatingly difficult to [break-fix](/fix/) for approximately the same sorts of reasons.
...
"Intelligent character recognition" (ICR), or "intelligent OCR", is a more advanced form of OCR that parses text beyond standard OCR, typically by training through [machine learning](/ml/). When properly trained, it can typically track handwriting styles and a vast variety of fonts, and an even more advanced form of ICR can work with identifying entire words altogether (IWR).
...
- [Password-protection](/authentication/) and [encryption](/encryption/) of the file itself.
- [Metadata](/files/) storage, for just about anything.
- Multiple layers for annotations, OCR, form fills, signatures, write-protection, and read-protection.
- Hidden text (functionally similar to [HTML's](/web-dev/) [image](/graphics/) alt text).
...
Unfortunately, [fixing](/fix/) PDF files only involves a few possible solutions:

1. Spool the PDF out through a software [printer](/printer/) to another PDF with the "Save to PDF" feature. This removes or merges most of the layers.
...
While development of the technology was relatively slow compared to others, barcodes became a ubiquitous part of retail by the 1980s once stores standardized Universal Product Codes (UPCs) to [track inventory](https://notageni.us/accounting/).
...
Ironically, QR codes have *not* been quick-response. While they've been adopted by many retailers (e.g., restaurants), they're much slower than a standard barcode, especially with a cellphone [camera](/camera/).
...
Unfortunately, it's not uncommon for information to parse incorrectly, even with [plenty of testing](/software-redesign/).
...
From [a design standpoint](/ux-ui/), adding barcode technology to an inherently social experience (e.g., restaurants, concerts) can create a *very* antisocial experience altogether, especially if people are conditioned to social interaction in a [culture](https://gainedin.site/culture/).
