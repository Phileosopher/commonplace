...
Because technology and software change around so frequently, programs have to change proportionally to the number of places they're "implemented".

Thus, features require maintenance, and require constant rework.

On the granular level, software development is about reliable [version control](cs-software-versionctrl.md), but good development requires [*constant* direct feedback](people-customerservice.md) from the users. This is harder than it sounds because the users often don't know what they want.

## Optimizing

Optimizing software can be unpleasant for software developers:
...
- Their ability to get their mind around the [algorithms](cs-langs-algorithms.md) and [flow of data](data-structures.md) is a new experience to them altogether (and therefore enjoyable).
...
One of the most significant sources of slowdown and [cybersecurity risks](cs-infosec.md) comes from using an off-the-shelf [framework](cs-langs.md) for an application:

- That framework will likely be sufficient (even to easily [scale](cs-distsys-enterprise.md)), but over time the developers of *that* framework can often add arbitrary features that will severely slow it down (and therefore the dependent project).
- Even without a framework, a sufficiently complex framework-free application will have an ad-hoc, [informally-specified](language-writing-documentation-cs.md), [bug-ridden](cs-software-redesign.md), slow implementation of half of a framework.
...
1. The software is finished. Outside of [security patches](cs-infosec.md) or [OS updates](cs-os.md), there is no further need to work on the software.
...
- If the lead developer [passes on](legacy.md), but [with a succession plan](hardship-death.md), nothing in the software itself will change, but the brilliant mind that [created that software](cs-software.md) will be difficult to replace.
...
Sometimes, however, a [large company](cs-distsys-enterprise.md) will buy out well-maintained software. From that point, the software will likely suffer one of several fates:

- In the pursuit of profit, the software will be modified to [track more data](cs-bigtech.md) or will develop a [freemium business model](legal-ip-floss.md).
- If the software was [open-source](legal-ip-floss.md), the developer or company will release a closed-source version of it, and another developer will subsequently [fork the software](cs-software-versionctrl.md) as another open-source implementation.
