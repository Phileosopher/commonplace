...
- An Apple-proprietary OS called MacOS. It had many great ideas (and has some of the best [UX](/ux-ui/) around), but has fallen in quality as of the early 2020's. It still comes on Apple desktops/laptops. It's based on Unix. Apple also has iOS for their mobile devices that was directly pulled from MacOS.
- A Microsoft-proprietary OS called [Windows](/windows/) that blatantly ripped off MacOS dominated the market in the 1980's because it was affordable and ran on most computers. As of 2022, it still dominates the desktop/laptop PC market.
- An [open-source](/floss/) kernel of [Unix](/unix/) called Linux. It has a vast variety of "distros", so it's a bit more open-ended.
- A Google-proprietary OS built on Linux called Android. It runs on most mobile devices that don't run iOS, though [lawsuits against Google's user data policies](/faang/) may change that from their hold in the early 2020's.

## Kernel design

There only two major approaches to OS design, but they all sit on a spectrum between two philosophies.

- A "microkernel" only manages what it has to: [CPU](/cpu/), [memory](/memory/), and IPC ("inter-process communication"). They are extremely portable and take up very little memory. Only specific processes run as "superuser". However, the hardware needs more "device drivers" to run peripherals, the processes will have to wait in line to get information, and the processes won't be able to access other processes.
...
[Windows](/windows/) and Mac are hybrid kernels that mix both of the architecture designs, while [Linux](/unix/) is strictly monolithic.
...
1. Hardware - deals with system "drivers" (such as the [screen](/screen/), [keyboard](/keyboard/), [mouse](/mouse/), [printer](/printer/), [scanner](/handwriting-ocr/), etc.).
2. Scheduling - manages the processes that pipe their way to the [CPU](/cpu/).
3. [Memory](/memory/) management - manages moving information back-and-forth between long-term storage and RAM.
...
It's worth noting that most operating systems don't display everything they *can* do, since [design concepts](/ux-ui/) would make that experience too complicated for the average user. There's a constant tradeoff between boring [command-line interfaces](/consoles/) and [elaborate graphical interfaces](/graphics/).

Because tech-savvy people and tech-retarded people have completely different opinions on what constitutes a workable interface and [file arrangement](/files/), there are a wide variety of interfaces to fit every need, with their own CPU labor costs involved.

## Programs

A program is a set of instructions that the computer can run.

When a program [creates variables and runs functions](/prog-basics/), the operating system can run allocate a memory stack for the program ("stack memory allocation"). Then, when the program ends, the operating system will deallocate that memory.
...
Each process starts *from* a [file](/files/), but many processes write *to* files or call *other* processes. Processes can "pipeline" data to another process and grab more input data. Since *all* the processes are pipelining, the entire computer can look a bit like a factory assembly line.
...
- Translation [protocols](/protocols/) that convert information into different formats for various needs.
...
- Notifications to indicate events from the system or relevant updated information from across a [network](/networks/).
- A shortcut system that allows quick access to programs, documents, and various scripts. In a [console-based GUI](/consoles/), quick-reference help documentation on the commands and their syntax.
- A "clipboard" system that allows copying something, then pasting it somewhere else.
- To prevent [malicious actors](/pentest/), a built-in scanner/verification process to [authenticate](/authentication/) the validity of a program that's about to run.

## GUI

Most programs can activate with commands. While the information once outputted through a [printer](/printer/), the OS now typically displays a [GUI](/graphics/) ("graphical user interface") on a [screen](/screen/).

We take *many* [GUI features](/ux-ui/) for granted in most consumer operating systems:
...
- A list of available programs, often [searchable](/algorithms/).
...
- Somewhere where programmers and IT technicians can view [system error logs](/software-redesign/).

## Development

Designing "native" OS [GUI-based](/graphics/) applications is a headache compared to [web apps](/web-dev/). Each operating system has its own [specific knowledge](/understanding/). Most of that knowledge is built around various methods to get software even working correctly. Plus, each proprietary OS (such as Apple) has an approval process that makes it even more complicated (e.g., App Store).
