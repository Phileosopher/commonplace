...
Most network [protocols](cs-standards.md) work over the internet. Since it's a worldwide network, various protocols and standards need to talk with each other, and their interoperability can be an *extremely* complicated endeavor.
...
To add to the above complexity, there are also [cybersecurity](cs-infosec.md) concerns from literally the entire planet. Every website essentially needs a [signed certificate](cs-infosec-authentication.md) to be trustworthy. Even then, it's absolutely crucial to develop [healthy web browsing habits](cs-infosec.md).

## Hardware

The hardware of the internet is a worldwide [network](cs-networks.md), mostly connected through various cables and [wireless signals](engineering-radio.md), along with some [satellite connections](logistics-navigation.md).
...
Web servers have layers of configuration:

1. The OS, which is usually Apache [Linux](cs-os-unix.md)
2. HTTP server software
3. [Database](database.md) management software
4. [Code](cs-langs.md)

Many use an "AMP server" configuration (**A**pache HTTP Server, **M**ySQL or **M**ariaDB, and **P**HP or **P**erl or **P**ython). The most popular one by far is Linux (LAMP), though quite a few are Windows-based (WAMP) or macOS (MAMP).
...
The "public IP address" is usually the internet-facing "router" or "gateway device" that the computer is connecting through, though it can be a [VPN's IP](cs-infosec.md) if you have one. Underneath that, there are multiple "private IP addresses" assigned to every computer on that network.
...
The RSS and Atom [protocol](cs-standards.md) are utterly simple: an updated ticker-tape of the most recent information available in a feed.
...
Sometimes, tech companies will *not* make their feeds freely available to enforce [a walled garden](cs-bigtech.md). But, in those instances, you can usually use a third-party update service for whenever a page changes or new content is distributed.
