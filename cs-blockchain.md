...
Computer files don't often stay the same. They're updated with new information, new [metadata](cs-files.md), things are moved around, [glitchy code is patched](cs-software-redesign.md), [updates roll out to handle changes from *other* software](cs-software-redesign.md), [databases](database.md) get cleaned up, and so on.

Naturally, across a [distributed system](cs-distsys.md) (such as cloud synchronization), this can pose extremely complicated challenges. There are a few general ways to sync:
...
Now, imagine combining the balancing of [peer-to-peer networking](cs-distsys-p2p.md) into this. Instead of 1 central server holding *all* the information, you'd need to add another layer of complexity. This is where "blockchain" comes in.
...
The nonces add onto other nonces, over and over, making data transfer mathematically arcane and horrifyingly inefficient. This entire setup is *very* [cyber-secure](cs-infosec.md), and creates very little room for hacking or error, but uses tons of electricity and processing power.
...
Crypto mining creates a severe workload on the drive and processor hardware, so many manufacturers have tried to limit crypto mining by making it work inefficiently or voiding the warranty, and [a few have actually used individuals' computers for their own gain without permission](cs-bigtech.md).
...
3. There was an attempt to make a soft [fork](cs-software-versionctrl.md) (that would keep it all together), but it didn't work because of a [security vulnerability](cs-infosec.md) with the consensus system, so the Ethereum Foundation had to create a hard fork called ETH.
...
However, it's hard to *clearly* define a comparison because most other monetary systems are spread across many other systems (e.g., banks, payment processors, US government, etc.). This problem *can* get better with optimization, but it takes [many redesigns](cs-software-maintenance.md).
...
At the same time, cryptocurrency can very effectively serve as the de facto currency standard if a government ever collapses into anarchy in a way that preserves its nation's [internet](cs-sofware-webdev.md).
