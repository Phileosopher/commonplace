...
Computer commands *could* distill down as [binary information](/logic/) (and, in practice, you always *can* examine them inside [memory registers](/memory/)) but that's absolutely tedious when you want to actually do something that *isn't* computer science.
...
- Instructions for how the [CPU](/cpu/) will work with various inputs or outputs (also called a "driver").
- Instructions for routine things the [operating system](/os/) must do frequently (like "refresh" the screen or check for new USB cables, also called a "process").
...
When you first [boot](/boot/) any computer, a few files are already running:

- Input file (stdin) - tracks what you're doing with the [keyboard](/keyboard/) (and occasionally the [mouse](/mouse/)).
- Output file (stdout) - feeds information to the [screen](/screen/)/[printer](/printer/)/[speakers](/speakers-mic/).
- Error file (stderr) - a database of [errors](/software-redesign/) with reference codes that specify how things broke.
...
A file can hold a *lot* of metadata, but not all metadata about that file is stored *inside* the file. It can often be contained in the operating system instead inside a [data structure](/data-structures/) (on Windows it's the master file table or MFT and Linux uses inodes).

File metadata often varies based on the file system, but it usually has at least most of the following:

- File [permissions](/security/), as well as the file's owner, and "access control lists/entries" that indicate which users can do what (i.e., write/read/rename/delete)
...
Files can also contain all sorts of multimedia, and it can get a bit complicated. Most codecs for [music files](/speakers-mic/), for example, don't usually contain *only* music. An MP3, for example, will have plenty of text data about the performer, [licensing/copyright](https://notageni.us/ip/) information, song lyrics, and album photos. For [memory allocation](/prog/) reasons of all the *possible* metadata, this can *really* pad out file sizes.

It's worth noting that there are often specific rules about what characters work in file names. All the alphabet is allowed, along with some special characters like commas, periods, and spaces. However, [regular expression](/prog-basics/) elements like "?" and "/" aren't allowed.
...
Since [cloud storage](/cloud/) became popular, some files can be labeled as a network resource instead of as a local one. While this can be profoundly convenient, it can confuse users who don't realize their files are *not* stored locally.
...
In the case above, File 1 is "fragmented" because not all the blocks are together. At one time, software such as Disk Defragmenter on [Windows](/windows/) would defragment, but now many [algorithms](/algorithms/) can predict fragmentation before it happens and accommodate it by either moving around blocks or writing to a different part of the disk.

Blocks can be any variety of [two-based](/logic/) sizes, ranging from a few kilobytes upwards to multiple megabytes. These blocks may have certain abilities depending on the file system:

- Internal snapshotting/branching - maintaining multiple copies of that block
- [Encryption](/encryption/) and compression
- Deduplication - getting rid of duplicates, which can save money in [enterprise systems](/enterprise/)
...
- Trim support - in [solid-state drives](/memory/), writes all the unused data to 0 to prolong the life of the drive

Because of the way [algorithms](/algorithms/) write blocks, you can save a *lot* of disk space if you group similar information together *before* compressing it for archiving. This can heavily affect [compiling sizes](/prog-features/) as well.
...
File systems exist within various [standards](/protocols/), most of them based on the type of [operating system](/os/) the computer uses. Some older file systems (e.g., FAT, FAT32) had a bad tendency to randomly lose data, and there are various file system features meant to make files safer and faster:
...
- Journaling - keeping a record file in long-term memory of tasks the CPU still has to do, very useful if the task is interrupted and can't continue, can sometimes *only* journal the metadata, a critical part of [cybersecurity](/security/).
- File change log - keeps a record of many changes to various files, can be localized to system files or apply to *all* files.
- Case-sensitivity - Some operating systems (like [Unix-based ones](/unix/)) are case-sensitive ("file.txt" *isn't* "File.txt"), while others (like [Windows](/win/)) are not ("file.txt" is the same as "FiLe.TXt"). Occasionally, a case-insensitive file system will still *preserve* the case-sensitivity.
...
Logical file systems fit squarely in the domain of [good UX](/ux-ui/), and getting rid of them to allow text-based entry and output is why [CLI](/consoles/) is still fashionable for tech-savvy users.
...
There are various hierarchical file systems ("exFat, ext2/3/4, jfs, gpfs etc.) which are each designed with different purposes in mind. The file system rules (and where to start the operating system's first programs) are stored in the [boot sector](/boot/).

Another convenience of a hierarchical file system is that "moving" a file on the same disk is simply renaming the file path, which may mean changing a few [memory references](/memory/) depending on the design of the hardware. This is why a drag-and-drop transfer can sometimes be instant. This, however, creates a [UX hangup](/ux-ui/) when moving across disks, since lots of data will take a while to transfer.

It's worth being aware of what a file is doing when you're copying, moving, and deleting:

- When you delete a file, the file is still there, but it gets marked for overwriting whenever the operating system needs more storage. Unless you actively use deletion software, [it'll still be there](/compliance/) if the operating system never overwrites it.
...
- Drag-and-drop with the mouse could mean *either* moving or copying depending on the [operating system](/os/) and where it's going, so it's better to use the cut or copy keyboard shortcut and then the paste shortcut.

However, there are some limits to hierarchical files based on how many bits were allocated to the task. At one point, [memory](/memory/) limits had a hard 256-character limit on the file. While this sounds like a lot, that character limit included the *folders* as part of that file name.
...
- This gets worse if you're dealing with [cloud storage](/cloud/) and [distributed systems](/dist-sys/), and *especially* with [virtual machines](/vm/).

## Database file systems

There's a less-frequent but important alternative file system that uses a [database](/database/) instead. However, while it could possibly *not* be hierarchical, it's essentially a top-level management system attached to a standard file system.
...
Beyond the [boot sector](/boot/), drives have "partitions" assigned for various purposes:
...
Except for some forms of [virtualization](/vm/), it's only sane to keep up to 1 operating system on 1 partition "volume". This evenly divides the tasks of an operating system in such a way that there's no conflict for what the CPU should do.

Volume management in [large-scale systems](/dist-sys/) can become challenging. It can help to use a native logical volume management file system (e.g., LVM-ext4, Btrfs), or it can be done higher up in the [OS stack](/os/) via [virtualization](/vm/).

Partitions are grouped based on the operating system.

- In [Windows-based](/windows/) systems, partitions are given arbitrary letters (C: as the first one because A: and B: were once for floppy disks, D:, E:, etc.).
- In [Unix-based](/unix/) systems, they're grouped under specific folders (and accessible with the "df -h" command).
