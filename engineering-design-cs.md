...
- Make sure the presented information is using any tracked [location data](logistics-navigation.md), and make it location-agnostic if there's a [VPN](cs-infosec.md) or no location. Generally, there's still usually an *approximate* nation or region to derive data from.
- If there is *any* information about the [operating system](cs-os.md), assume the user is using that particular OS. Either way, give a simple selection for *all* available OSes.
- If the user has provided information, use it to tailor all relevant [algorithms](cs-langs-algorithms.md) of the software (e.g., their genre preference should define *all* the content).
...
[Software engineers](cs-langs.md), by the nature of their trade, do not need great [communication skills](people-conversation.md) and often have limited [people skills](people-image.md), so many computer design elements have been, from their inception, fundamentally obtuse.
...
[Cybersecurity](cs-infosec.md) design, by contrast, is simply the *reverse* of good design: instead of communicating what a user can do, the designer is obscuring and *hiding* things instead.
...
With software, it's not difficult to include help files and browser-based [hyperlinks](cs-sofware-webdev.md) to lead the user to what they need. With hardware, it's worth including both simplified and advanced documentation for different levels of users, or to at least maintain a permanent archive hyperlink built onto the package.
...
[Software development](cs-software.md) has a unique, nasty problem:

1. First, the software must be designed, then [tested a *lot*](cs-software-redesign.md) to ensure it actually works.
...
- The people who design software are human beings who [may not want the users' best interests](cs-bigtech.md), and have reason to employ "dark patterns" to make some elements difficult to access (e.g., unsubscribe, delete account, remove connection).
...
- The only way to keep an inherently complex product like software or a computer simple is to *constantly* investigate how to merge or remove obsolete features. Just make sure you explicitly document those changes, and allow people to roll back [those updates](cs-software-redesign.md) if they need.
...
Generally, large-scale changes can force an immobilized design experience. Back when [Windows 95](cs-os-windows.md) dominated the OS market, people could tweak their wallpaper, colors, and sound effects, but most granular control is constrained in most mainstream [OS's](cs-os.md). Having the freedom to keep a customizable interface would require exponentially more work, or the quality would suffer.
...
- Requiring the user install extra software or accept extra [permissions](cs-infosec-authentication.md) that are convenient for the developer, but often involve [tracking data that the user may not want tracked](cs-bigtech.md).
...
- They'll be *much* quicker to adopt trends, and the speed of adoption determines how fast it'll get abused. It can lead to absolutely absurd interfaces until it's [patched later](cs-software-maintenance.md).
...
- Asking for [permissions](cs-infosec-authentication.md) or for the user to download software is highly convenient for the developer, even while it's annoying for the user, and that selfishness bleeds into the software experience.
...
- If it's consumer-facing (e.g., [web design](cs-sofware-webdev.md)), the push for simplicity and ease-of-use will make arcane systems hiding behind deceptively simple visual elements.
- If it's more technical or organizationally facing, the constant [feature bloat](cs-software-maintenance.md) will make the interface cluttered with all the extra edge cases present for the user.
...
In technology, the "slow" element is the user's understanding. More than anything else, the slowest part of any designed computer that isn't a [large-scale distributed system](cs-distsys.md) is the user.
...
- A "download" button that's near the *actual* download button that typically contains [a virus](cs-infosec.md).
...
- Adding [advertisements](marketing.md) in between users' content, with [algorithms](cs-langs-algorithms.md) that tailor those ads to be as associated as possible to what the user may want to have.
