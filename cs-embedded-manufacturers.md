...
Pis run on [Linux](cs-os-unix.md) with a specialized distro for it, so they work with a vast variety of [programming languages](cs-langs.md).
...
- [The Arduino](https://www.arduino.cc/en/Main/Products) is a bit weaker than the Pi, but doesn't have the overhead or complexity of an entire [operating system](cs-os.md), so it's easier to scale than the Pi.
...
- Like the others, they have single board computers, as well as a cluster board for [distributed systems](cs-distsys.md).
...
- ODROID-GO has a tiny screen with it and is designed as a portable [emulator console](cs-distsys-vm.md).
...
- [Printers](engineering-printers.md) and [scanners](cs-ocr.md)
- Old [networking](cs-networks.md) equipment like routers, modems, telephones, and fax machines
