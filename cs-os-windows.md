...
- Nearly everything in [the Unix world](cs-os-unix.md) was [open-source](legal-ip-floss.md) (i.e., paying for service agreements or hardware more than software).
...
Before Windows was MS-DOS, a [command-line interface](cs-cli.md) released in 1981.
...
- Windows was more user-friendly by integrating use of the [computer mouse](cs-mouse.md) for *everything*.
- It was widely popular, and became the standard operating system on most computers for at least a few decades.

Microsoft Windows, up through Windows 3.1, was originally a blatant ripoff of [macOS](cs-os-apple.md), and copied most of the conveniences of Apple's [design](engineering-design.md). Since it was more affordable than Macintosh computers, and ran on almost anything, Windows was able to [dominate the operating system market](cs-bigtech.md). Even in the 2020s, [Linux](cs-os-unix.md) has made some good contenders, but Windows is still the convention for most non-mobile personal computers.

Windows forked a version of Windows called Windows NT in 1993.

- It was designed for [commercial use](cs-distsys-enterprise.md) (i.e., multiple users across potentially multiple computes with tons of tracking).
...
Windows 95 changed out its design from its 3.1 roots, since Microsoft didn't want a [lawsuit](cs-bigtech.md) from Apple.
...
- Instead of using MS-DOS, the computer could [boot](cs-boot.md) straight into Windows.
...
- The OS ended up as nothing more than a visually polished Windows 95 with a hybrid [16/32-bit architecture](cs-cpu.md).
- It was meant as a holdover until Windows XP, and should have undergone more [debugging](cs-software-redesign.md).

The high point of Windows was indisputably with Windows XP, released in 2001.

- It was thoroughly stable, kept driver backups in case a device driver failed, had a Prefetch feature that preloaded the [RAM](cs-memory.md) with frequently accessed programs, and ran strictly on a 32-bit architecture.
- It borrowed heavily from Windows NT, and many [enterprise-grade](cs-distsys-enterprise.md) features were available inside the system.
...
- They also ended long-term support for it, meaning that [Ubuntu](cs-os-unix.md) is a better option for a low-spec computer.
...
- On the back-end, Windows 11 was Windows 10 that sent more user data, which has been [trending away from acceptable](cs-bigtech.md) for many people.

## .NET

On the development side, one of the most significant portions of Windows comes through the .NET framework. It's a Microsoft-specific framework that builds features and elements for an ever-increasing variety of [programming languages](cs-langs.md).
...
ASP.NET was designed to expand .NET into [web apps](cs-sofware-webdev.md) by using the C# language instead of JavaScript. Developers have mixed opinions about it: it's trying to create a fire-and-forget set of solutions, but particularly intelligent [software designers](cs-software.md) will find it irritating.

## Updates

Windows Update was once a useful feature. It released patches that [fixed bugs](cs-software-redesign.md).
...
- To cut costs, Microsoft replaced that department with Windows Phone, then used [virtualization](cs-distsys-vm.md) from then on to track any issues before sending it out.
...
Operating systems need constant [redesign](cs-software-redesign.md) to stay current to the changes in technology. They have to accommodate better [memory](cs-memory.md), more streamlined [programming languages](cs-langs.md), and [UX improvements](engineering-design.md).

At the same time, updating takes time and money. Most PCs have come pre-installed with Windows, so the executives who run Microsoft don't necessarily need to work at creating an ideal user experience, unless everyone were to leave for [Linux](cs-os-unix.md).
...
Further, Windows 11 now demonstrates their primary profit motive comes less from bringing a meaningful user experience than from [collecting user data](cs-bigtech.md).

However, for anyone who finds Windows irritating enough, Linux has reached a state of maturity where it is now universally better than Windows. In fact, many Windows-based [software emulation](cs-distsys-vm.md) through Linux (typically via the Wine software) is *better* than running it natively on Windows!

## Not broken

At the same time, Windows isn't inherently defective.

- The built-in Windows Defender [antivirus software](cs-infosec.md) is well-designed, and none of the other antivirus software products are particularly necessary if you're [avoiding dodgy websites and not running malicious software](safety-computers.md).
