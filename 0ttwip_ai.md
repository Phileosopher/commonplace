...
- Makes [decisions](https://gainedin.site/decisions/) to play [games](/game-dev/) the way a human does.
- Using [language](https://gainedin.site/language/) in writing that matches what a person would likely write.
...
The first type of AI was created in 1965, and was called an expert system. It operated on a set of if/then rules based on a body of knowledge. It works well-enough for things with a limited set of choices and purely logical decisions (e.g., playing chess, "paper or plastic", [some video games](/game-dev/)).
...
[Machine learning](/ml/) is the new AI. If a machine learning [algorithm](/algorithms/) is capable of detecting something, it is also capable of reproducing it.
...
- Reviewing [business contracts](https://notageni.us/contracts/) for compliance and [risk assessment](https://notageni.us/risk-mgmt/), which can also mean building them as well. It would need re-training for each purpose, though.
- Observing [trends](https://gainedin.site/trends/) across [data](/database/), which *theoretically* can allow the algorithm to make future predictions.

## Limits

We're a very long way off from reproducing human behavior. Since [feelings](mind-feelings.md) are so messy, computers must be programmed to observe events and then appear like they're reacting to them. This often involves shedloads of [calculus](/calc/) and [statistics](/stat/), but will always feel inauthentic to us because it'll miss idiosyncratic aspects of intuitive context.

Even with all the benefits of [machine learning](/ml/), AI can be ridiculously stupid, which is heightened by the gap between expectations and reality:
...
Also, depending on how we view the [soul](https://gainedin.site/religion/), there's a hard philosophical "wall" that AI can never breach:
...
But, that's the domain of [religion](https://gainedin.site/religion/) and [speculation](https://gainedin.site/imagination/). As it stands, life is impossible through AI.
