...
The electronic gaming industry is *huge*. It's been pulling in more money than movies for some time now, and as technological implementations like ever-increasing [processing power](/cpu/) and [VR](/vr/) become more affordable, it's only a matter of time before games become the primary medium of electronic entertainment.

Once technology opens up AR (augmented reality) interfaces, game design directly corresponds to real-world implementations. Further, the technology that feeds into games (e.g., [graphics](/graphics/) software) often feeds into other more practical outputs, such as [3D printing](/3d-printer/) and [general-purpose AI](/ai/).
...
The 1970s represented some pioneering ideas in gaming, mostly dominated by Atari. However, processor limitations left the UX somewhat lacking, and the most timeless experiences from that era came through [vector graphics](/graphics/) (e.g., Missile Command, Asteroids, Battlezone).
...
Somewhere before 2010, game innovation had peaked its [trend](https://gainedin.site/trends/).
...
- They tend to simply be an [improved version](/version-control/) of the previous game, with borrowed "game mechanics" from other games.
- For [marketing reasons](https://notageni.us/marketing/), the game would add easily-implementable gimmicks or [hacks](/hacking/) to advertise a feature:
...
- The limited budget meant the games were [meaningful](https://gainedin.site/meaning/), but comparatively short.
...
While [web-based](/web-dev/) PC game platforms in the late-2010s (e.g., Steam, GOG) have leveled the competitive playing field a bit, there's still a stark discrepancy between the risk-taking [artist](https://gainedin.site/creativity/) indie game developers and the mass-produced corporate AAA game development team.
...
Game development is all effective aspects of [software development](/software-design/), but requires *many* [specialized](https://gainedin.site/specialization-cities/) roles that would otherwise be optional or unnecessary in most other software domains:

- Direct game designers, which can branch out into gameplay, levels, monetization, [usability/UI](/ux-ui/), cinematic cutscenes, [narrative](https://gainedin.site/stories/), [writing](https://adequate.life/writing/), and [missions/quests](https://gainedin.site/purpose/).
- Programming roles that branch out into all standard software development, including [graphics](/graphics/), [animation](https://notageni.us/arts/), [AI](/ai/), [servers](/dist-sys/), physics, pipeline engineers, build engineers, testing, visual effects, databases, [language-specific](/langs/) roles, and roles specific to game engines.
- A wide variety of [art](https://notageni.us/arts/) roles including 2D and 3D versions of environments, animation, rigging, motion capture, textures, lighting, 2D characters, 2D concept art, 3D camera, storyboarding, and both UI *and* UX.
- [Quality assurance](/software-redesign/) roles, especially play testing.
- All aspects of [management](/mgmt/), for *everyone*, with subdivisions for product management and studio administration.
- Community support roles, such as [customer support](https://notageni.us/cust-svc/).
...
1. The [visual elements](/ux-ui/) are usually far more [vast](/graphics/), and the designers need to worry about both animation *and* static elements.
2. The [audio](/speakers-mic/) must be synchronized to the visual experience, so it has to be correctly designed, from the sound effects to the soundtrack.
3. Games require *immediate* feedback from the input peripherals (such as the [keyboard](/keyboard/) or [mouse](/mouse/), and now [VR](/vr/)). While people can endure a 1-second delay in many other programs, 0.1 seconds is enough to make a janky game.
4. For many games, you need an elaborate [mathematical framework](/algorithms/) to keep all the visual and audio elements working in tandem. This is *much* more than most other programs.
5. Across the internet, the demand for rapid-response [networking](/networks/) in many genres is a perpetual need, for *both* player input and [visual output](/screen/).
6. In multiplayer games that need computer players (e.g., a 2v2 game with 3 humans), the [AI](/ai/) has to have enough logic to permit the player to feel like the "bot" is human-like.
7. [Graphics](/graphics/) technology is often driven by large-scale game developers, who build incredibly elaborate designs for their games that need ever-increasing [processing power](/cpu/). This ranges from the character designs all the way to expansive and beautiful "skyboxes" and long-distance rendering to recreate extremely large worlds.
...
- However, in the 1990's Nintendo Entertainment System, the developers had to [hack](/hacking/) the CPU to halt drawing one portion with a predetermined sound that forced the hardware to stop, then used the hardware's "collision detection" bit to draw the other part.
- Even through to today, vintage [game emulators](/vm/) are notoriously difficult to build because the software must reproduce the individual clock cycles and constraints of the original hardware.

## Netcode

Across a network, games have extremely stringent requirements for input, and heavy requirements for output. From frame to frame across a [network](/networks/), the game must match on *both* computers.

Online games are typically designed as "deterministic", which means two instances of the same program will create *precisely* the same outputs with the same inputs. Even random numbers can be [algorithmically](/algorithms/) derived deterministically. This gives the added convenience that a replay of a game can simply be the inputs mapped to timing inside a game, but also allows other games to reconstruct a match even with [network](/networks/) latency.
...
Delay-based netcode will intentionally slow down the input from registering on the frame it was used. Instead, it'll update a few frames later (each frame represents [~1/60 of a second](/screen/)). While a few people well-trained in games may notice the delay, clever game design and the delay staying consistent make delay-based netcode work fine. Unfortunately, networks are rarely consistent: any slowdown in the network will slow the game down, even with plenty of tricks that predict network behavior, and physical distance will have a profound impact on gameplay.
...
- The only exception to this is specific game genres (e.g., visual novels, some [VR experiences](/vr/)), but advancing the interface doesn't define as a "game" (i.e., there should be a capacity for [making decisions](https://gainedin.site/decisions/)).

Game [UX](/ux-ui/) should be non-intrusive, where the user will feel or observe what they should do *without* explicit instruction. To avoid text boxes with instructions or written signage, there are several tools to guide the player:
...
- A gameplay loop is a hot-and-cold cycle through a developed [habitual](https://gainedin.site/habits/) routine.
...
2. The game must be challenging enough: too much and the player will "ragequit", too little and the player will be bored. The challenge must come from the player's *interaction* with the game, not from [software errors](/software-redesign/) or arbitrary game design that stretches out time.
3. The game must make the person curious, or they won't want to keep playing. This sometimes come from the game's [story](https://gainedin.site/stories/), but more often comes from the functionality of what the player controls and the possibilities that it opens (e.g., seeing if they can jump across a normally impassable large chasm using a weapon that launches them backward).
...
In online games that have in-game currency and permit players to trade with each other, the emergent nature of [economic bartering](https://gainedin.site/economics/) arises.
...
Most games, with the exception of some skill or puzzle games, are driven by a [story](https://gainedin.site/stories/).
...
- Unless the game designer is particularly [creative](https://gainedin.site/creativity/), there must be an enemy or set of enemies.
...
1. [Writing high-quality stories](https://gainedin.site/stories/) requires a completely different skillset and talent than coding a game.
...
Designing a good game requires [graphics development](/graphics/), both in 2D and 3D, as well as tons of [programming](/prog-basics/), but much of it is automated. It once required direct programming for just about everything, meaning it used to be *far* more challenging.

Today's game development has many drag-and-drop interfaces that turn game programming into far more of a dynamic moving visual art than simply a highly-involved domain of [software development](/software-design/).

The simplicity of making games now has created a heavily saturated market of game developers, and the increased demand from all the people who want to play games has made it a heavily competitive domain. Developers are *constantly* stealing [intellectual property](http://notageni.us/ip/) from one another, and most of the ideas from AAA game developers are blatantly pulled from an indie developer who wouldn't be able to sue for copyright infringement. In the long-term, AAA games spend so much on mass [marketing](https://notageni.us/marketing/) that they receive credit for new features or gameplay elements.

For each genre or style, game developers generally move through a [trend](https://gainedin.site/trends/) of motivations that drive them into the industry:

1. An independent developer [creatively](https://adequate.life/creativity/) designs their heartfelt art. The game is beautiful, but often unpolished from the fact that they're pioneering the entire thing and don't have the staff to clean it up.
...
3. Optionally, give the user a wide variety of possible rewards to provide the experience of making [choices](https://gainedin.site/decisions/).
...
6. Eventually, extend the experience into real-world [economics](https://gainedin.site/economics/) by making real-world currency necessary to advance the game.

Most of the time, game developers will [scale up](/mgmt-changes/) by having *other* developers make the games, contributing their accumulated experience in [ideas](https://adequate.life/creativity/)/[marketing](http://notageni.us/marketing/)/[debugging](/software-redesign/), then publishing with their brand attached to it and likely a royalty for sales.

## Trends

The electronic games industry has gone through many [trends](https://gainedin.site/trends/) that no longer exist, though they tend to pop up in weird places.

To change difficulty or configurations in the early 1980s, Atari games had simple toggle switches before starting a game that required [documentation](/documentation/) to decipher. By the 1990s, this difficulty switch eventually dissolved to an Easy/Medium/Hard selection when starting a game, and became an obscure feature hidden in the settings by the 2010s or simply nonexistent.

For some time in the mid-1990s, game installers took advantage of multimedia graphics, and started the [UX](/ux-ui/) into the game as soon as someone inserted a disc. The Command & Conquer: Red Alert game may have been the most interesting one, with [full animations of a rocket going through stages of loading for getting fired as the installation progressed](https://www.youtube.com/watch?v=Ri-VlJVxc-g).

In the mid-1990s, game CDs had a then-staggering memory capacity, so full-motion [video](/graphics/) could fit on it (though it was pretty lame at first). One form of game was called QuickTime events, named after Apple's then-new QuickTime protocol. The idea was that a full video would play, and the player would have to press a button when prompted. The pinnacle games of this were Dragon's Lair and Night Trap that were literally *nothing* but QuickTime events. They weren't particularly fun to play, but were novel enough to draw attention.

[Memory](/memory/) constraints limited most games in the early and mid 1980s, so they all had some relatively similar features. Later games (especially after the 2010s) would imitate them for nostalgic/artistic effect:

- No discernible plot, or it was in the supplied [manual](/documentation/). Often, the goal of the game was a recorded high score, and maxing out the score (which was a feat of extreme skill or [exploitation](/hacking/)) would make the game crash.
- To pad out play time, the game was absurdly and unfairly difficult, even on the easiest setting.
- To help with [play-testing](/software-redesign/), certain button combinations could give the player more lives, invincibility, swap out the sprites with other sprites, or skip ahead levels. These stayed in the game afterward and became known as cheat codes.
...
Online games tend to use "leaderboards", online high score systems that track *everyone* who played the game. Since this number can be thousands or a few hundred thousand players, it's not particularly enjoyable for most players, and most leaderboards eventually devolve into [hackers](/hacking/) who exploited the game to achieve an inhumanly high score.

Since the late-2000's, most [graphics](/graphics/) trends have moved toward trying to reproduce the constraints and artifacts from video [cameras](/camera/). This trend hasn't stopped yet as of 2023, and has even gone as far as reproducing grit and water spots on the screen, which destroys the immersion of controlling a proxy character.

One trend that started around the time Fortnite became popular in the late-2010's is to release the multiplayer game for free, then charge for the single-player experience. This allows the developer to collect user data, both to [reconfigure](/software-redesign/) the software to make it more addicting or to [sell the data outright](/faang/).

## Versioning

The ambitious nature of games makes its [software lifecycle](/version-control/) more involved and complex than most non-game software.

Ever since the internet (and since online stores like Steam that also manage [updates](/software-redesign/)), the game will often release in an unfinished state. At its simplest, the game will have glitches during gameplay, but many developers intentionally release games in so-called Early Access, which is effectively a [remarketed](https://notageni.us/marketing/) "alpha" or "beta" build of the game.

For many [open-source](/floss/) games, they're *never* officially done because of the constant improvements added by independent developers.
...
- In fact, some games are entirely *[free](/floss/)*, with the expansions being most of the game's content.
