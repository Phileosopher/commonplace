...
Everyone on the planet with a computer has at least heard of Microsoft Windows, but most people aren't aware of much more than its brand and [marketing](http://notageni.us/marketing/).
...
- Nearly everything in [the Unix world](/unix/) was [open-source](/floss/) (i.e., paying for service agreements or hardware more than software).
...
Before Windows was MS-DOS, a [command-line interface](/consoles/) released in 1981.

- MS-DOS rapidly became ubiquitous with personal computing because Microsoft's creator, Bill Gates, scored a nonexclusive [contract](https://notageni.us/contracts/) with IBM.
- To IBM's misfortune, the contract meant Microsoft was [licensed](/floss/) to run MS-DOS on *anything* that could support it, and consumers didn't really care.

Windows 1.0 was released in 1985, and ran as software on top of MS-DOS.

- Windows was more user-friendly by integrating use of the [computer mouse](/mouse/) for *everything*.
- It was widely popular, and became the standard operating system on most computers for at least a few decades.

Microsoft Windows, up through Windows 3.1, was originally a blatant ripoff of Mac OS, and copied most of the [conveniences](/ux-ui/) of Apple's [design](/ux-ui/). Since it was more affordable than Macintosh computers, and ran on almost anything, Windows was able to [dominate the operating system market](/faang/). Even in the 2020s, [Linux](/unix/) has made some good contenders, but Windows is still the convention for most [software developers](/software-design/).

Windows forked a version of Windows called Windows NT in 1993.

- It was designed for [commercial use](/enterprise/) (i.e., multiple users across potentially multiple computes with tons of tracking).
...
Windows 95 changed out its design from its 3.1 roots, since Microsoft didn't want a [lawsuit](/faang/) from Apple.

- Windows 95 had an industry-changing [interface](/ux-ui/) because it was the first OS to use a child-friendly setup.
...
- Instead of using MS-DOS, the computer could [boot](/boot/) straight into Windows.
...
- The OS ended up as nothing more than a visually polished Windows 95 with a hybrid [16/32-bit architecture](/cpu/).
- It was meant as a holdover until Windows XP, and should have undergone more [debugging](/software-redesign/).

The high point of Windows was indisputably with Windows XP, released in 2001.

- It was thoroughly stable, kept driver backups in case a device driver failed, had a Prefetch feature that preloaded the [RAM](/memory/) with frequently accessed programs, and ran strictly on a 32-bit architecture.
- It borrowed heavily from Windows NT, and many [enterprise-grade](/enterprise/) features were available inside the system.
- It was also "backwards-compatible" to other Windows software with Compatibility Mode.
- It also had multiple GUI updates that took advantage of the improved [graphics processing](/graphics/) from the early 2000s.
...
Windows' [trend](https://gainedin.site/trends/) of improvement wobbled around, starting with Windows Vista, released in 2007.
...
- They also ended long-term support for it, meaning that [Ubuntu](/unix/) is a better option for a low-spec computer.
...
- The system was filled with horrible [UX](/ux-ui/) mistakes: getting rid of the familiar Start menu, mapping the mouse to the swipe features that a touchscreen usually had, using stock photography without any labels, and burying extremely common tasks 4-5 menus deep.
...
- On the back-end, Windows 11 was Windows 10 that sent more user data, which has been [trending away from acceptable](/faang/).

## .NET

On the development side, one of the most significant portions of Windows comes through the .NET framework. It's a Microsoft-specific framework that builds features and elements for an ever-increasing variety of [programming languages](/langs/).

.NET also has its uses on Linux and Mac, but since it *is* Microsoft, the .NET framework is inextricably married to Windows. It's a good framework generally, though it's very much [proprietary](/floss/).

ASP.NET was designed to expand .NET into [web apps](/web-dev/) by using the C# language instead of JavaScript. It has...mixed opinions. On one side, it's trying to create a fire-and-forget set of solutions, but particularly intelligent [software designers](/software-design/) will find it irritating.

## Updates

Windows Update was once a useful feature. It released patches that [fixed bugs](/software-redesign/).
...
- To cut costs, Microsoft replaced that department with Windows Phone, then used [virtualization](/vm/) from then on to track any issues before sending it out.
...
Operating systems need constant [redesign](/software-redesign/) to stay current to the changes in technology. They have to accommodate better [memory](/memory/), more streamlined [programming languages](/langs/), and [UX improvements](/ux-ui/).

At the same time, updating takes time and money. Most PCs have come pre-installed with Windows, so the executives who run Microsoft don't necessarily need to work at creating an ideal user experience, unless everyone were to leave for [Linux](/unix/).
...
Further, Windows 11 now demonstrates their primary profit motive comes less from bringing a meaningful user experience than from [collecting user data](/faang/).

However, for anyone who finds Windows irritating enough, Linux has reached a state of maturity where it is now universally better than Windows. In fact, many Windows-based [software emulation](/vm/) through Linux (typically via the Wine software) is *better* than running it natively on Windows!

## Not broken

At the same time, Windows isn't inherently defective.

- The built-in Windows Defender [antivirus software](/security/) is well-designed, and none of the other antivirus software products are particularly necessary if you're [avoiding dodgy websites and not running malicious software](/cysec/).
