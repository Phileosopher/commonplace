...
This has meant, without exaggeration, that the average computer of 2000 is about 1,024 times faster than a computer made in 1980. The nature of it means computers get cheaper and faster.

This has slowed significantly. Once [processors](cs-cpu.md) started getting small enough, the electrons started interfering with each other. Due to heat management issues, processors can't run at an economical speed much faster than 4.8 MHz.
...
1. The user's [purpose](purpose.md) has expanded that they need a particularly powerful one to do something (most often for [playing games](cs-software-gamedev.md) or professional media creation).
2. They need to add a peripheral and the computer is so old that it can no longer support a modern [standard](cs-standards.md), even with a converter/adapter.
...
Computers provide [*many* career options](jobs-2_goals-cs.md), and [some organizations](cs-embedded-manufacturers.md) decided to do something different with them. Instead of selling top-dollar computers, they shaved down as many features as possible to create the cheapest possible computer.
...
Due to the price point and accessibility for tinkering, there are [many potential uses](cs-embedded-ideas.md) for these embedded computers.
...
No matter what you're poking around with, you *must* be willing to explore [Linux](cs-os-unix.md).

Firstly, the cost of a [Windows](cs-os-windows.md) license for that device is about as much as a low-tier Windows device.

Second, Windows requires so much processing power and memory [compared to most Linux distros](cs-os-winunix.md) that the computer would run very slowly, if it runs at all.
