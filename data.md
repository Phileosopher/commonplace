...
While computers are always too dumb to understand the essence of truth, all computer data is some form of absolute. Even when it's vaguely expressed or is intended to have an intuitive component (e.g., [AI](cs-ai.md)), it *must* have a concrete aspect to it for the information to be encoded.
...
3. That math can capture a *lot*, including [text](cs-ocr.md) and [visual information](engineering-camera.md), using a vast variety of standards and procedures.
4. The numerical values can be manipulated, expressed, transferred, and whatever else that the computer is [programmed](cs-langs.md) to do.
...
- Depending on the [programming language](cs-langs.md) and [compiler](cs-langs-compilers.md), it returns 3 possibilities if it's operated on:
  1. Ignores it (e.g., "John's email is ")
  2. Prints it (e.g., "John's email is null")
  3. Gives a NullPointerException [error](cs-software-redesign.md)
...
- As of writing this, they can be anything between positive and negative 2,147,483,648 (2^21 because of [how bits work](cs-alu.md)).
...
- They're *really* useful for tracking [keyboard](cs-keyboard.md) presses (such as a [computer game](cs-software-gamedev.md)).
- They come through symbolically representing certain numbers as specific characters (e.g., [U+0034 for "A"](cs-keyboard.md)).
- They can't be operated on at all.

Date/Time

- Date and Time is a variously expressed form of capturing time-based values, based on [set standards](cs-standards.md).
...
- Refs (short for references) are pointing to other things such as [memory](cs-memory.md) locations, [variables](cs-langs.md), or [network locations](cs-networks.md).
...
- Technically, other data types like Boolean logic are frequently enumerated types, depending on how the [programming language](cs-langs.md) sets it up.

## Complex data

There are many, many, many more classifications of data, which typically comply to a wide variety of [industry standards](cs-standards.md), and these data primitives can assemble into many more advanced [data structures](data-structures.md).
...
Even then, data types can often be standardized more locally, such as in a programming language (e.g., graphs exist as a standard in the Datalog [programming language](cs-langs.md)).
...
- [Audio](engineering-audio.md): MP3, WAV
- Image: JPG, GIF
...
Incoming data doesn't always conform to what may be needed for a computer's input, so many [algorithms](cs-langs-algorithms.md) are designed to "normalize" that data. This may include:
...
Data itself is stored in [memory](cs-memory.md), and computers only do a few possible things with it:

1. Wait and do nothing, with the information held for later use.
2. Do further logical things with it, based on what a [program](cs-langs.md) is instructed to do.
3. Output the information somewhere. It may be nearby (e.g., a [screen](engineering-screen.md)) or sent out to another computer via a [network](cs-networks.md).
