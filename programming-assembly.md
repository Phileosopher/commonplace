...
[Higher-level languages](programming-basics.md) must run through a compiler first to become assembly code, *then* convert through an assembler that converts it to binary. The system flows information through a distinct process:
...
Generally, assembly code follows [normal code procedure](programming-basics.md) like white space not being counted and comments, though the one-to-one relationship means it's far more rudimentary than higher-level languages.
...
- There are no "[variables](programming-basics.md)" in the conventional sense, since there are only binary memory registers that can get duplicated as separate references.
