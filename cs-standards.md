...
- USB - Universal Serial Bus, a standard swappable cable for most computers designed to be as near-universal as possible:
  ...
  - USB Type B - rectangular form factor with tapered ends on one side, less frequently used (e.g., [printers](engineering-printers.md), [scanners](cs-ocr.md)) and typically is Type A on the cable's other side
  - USB Mini - small form factor typically for smaller devices (e.g., [cameras](engineering-camera.md), [scanners](cs-ocr.md)), largely deprecated for USB Micro
...
- SFP - Small Form factor Pluggable, a standard for [large-scale servers](cs-distsys-enterprise.md)
...
[Console/terminal](cs-cli.md) data protocols:
...
- Some protocols verify email [domains](cs-sofware-webdev.md) for validity, such as DMARC (Domain-based Message Authentication, Reporting and Conformance)
