...
Computers have evolved over the years to the point that an [operating system](cs-os.md) is a standard feature. But, before the operating system even starts, there are a few elements that must activate first:
...
4. Identify the boot device. This was once a floppy disk that had the [operating system](cs-os.md) directly on it to load to RAM before swapping to other programs, but is now typically a hard drive (or sometimes a supplementary flash drive).
...
7. The boot sector of the partition loads code that starts the [operating system](cs-os.md) itself.
...
It's worth noting that resizing and moving partitions is *not* trivial, as opposed to [virtual drives](cs-distsys-vm.md) that have arbitrarily-set sizes:
...
3. Load the boot manager into memory and run either an OS loader or the [OS kernel](cs-os.md).
...
3. Partition entries in LBA 2 through LBA 33
   - each partition entry has the following:
     ...
     - A 32-bit cyclic redundancy check (CRC32) [checksum](cs-infosec-authentication.md).
...
One interesting aspect is that a BIOS/UEFI is a base-level stripped-down operating system meant for simple tasks. This means that there is a *lot* of legacy [code](cs-software.md) in there, and it's ridiculously slow with respect to clock cycles. If you ever wonder why booting takes a while after you press the power button, it's because the manufacturers don't care to delete the 99% of code that's irrelevant.

This *does* represent a [cybersecurity risk](cs-infosec-pentest.md), which is why UEFI's Secure Boot can sign the boot with a [cryptographic key](encryption.md), and why Windows computers tend to use Trusted Platform Module (TPM). It's possible (and a good idea) to harden Linux computers with Secure Boot.
...
On an MBR disk, there's a gap just after the MBR and before the first system partition. This permits a bootloader to exist in that gap for the user to select a [partition](cs-files.md) (and, naturally, an [operating system](cs-os.md) on it).
...
With GPT systems, the first sector is blocked off to error out legacy systems that use it, then use LBA 1 to boot from the primary GPT header. In a Linux system, you can access the EFI system partition (ESP) at /sys/firmware/efi, with the [file](cs-files.md) having a .efi extension.

## Features

Booting isn't always so straightforward. Instead, in an [enterprise](cs-distsys-enterprise.md) situation you can use a remote-access boot command with a pre-boot execution environment (PXE):

1. The computer lets the [DHCP](cs-standards.md) server know that it's doing a PXE boot.
...
The operating system will use a variety of files, which will have different arrangements depending on [how the file system is designed](cs-files.md).
