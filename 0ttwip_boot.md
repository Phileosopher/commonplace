...
4. Identify the boot device. This was once a floppy disk that had the [operating system](/os/) directly on it to load to RAM before swapping to other programs, but is now typically a hard drive (or sometimes a supplementary flash drive).
...
7. The boot sector of the partition loads code that starts the [operating system](/os/) itself.
...
1. 446 bytes of master boot code that gives instructions about what's elsewhere on the disk and how it's [encrypted and encoded](/encryption/).
2. 64 bytes for a "disk partition table", which is a [database](/database/) of primary partitions. Each primary partition is 16 bytes, so you can only have up to 4 per disk without recursively [hacking](/hacking/) the primary/extended partition system.
...
It's worth noting that resizing and moving partitions is *not* trivial, as opposed to [virtual drives](/vm/) with their arbitrarily-set sizes:
...
3. Load the boot manager into memory and run either an OS loader or the [OS kernel](/os/).
...
3. Partition entries in LBA 2 through LBA 33
   - each partition entry has the following:
     ...
     - A 32-bit cyclic redundancy check (CRC32) [checksum](/authentication/).
...
One interesting aspect is that a BIOS/UEFI is a base-level stripped-down operating system meant for simple tasks. This means that there is a *lot* of legacy [code](/software-design/) in there, and it's ridiculously slow with respect to clock cycles. If you ever wonder why booting takes a while after you press the power button, it's because the manufacturers don't care to delete the 99% of code that's irrelevant.

This *does* represent a [cybersecurity risk](/pentest/), which is why UEFI's Secure Boot can sign the boot with a [cryptographic key](/encryption/), and why Windows computers tend to use Trusted Platform Module (TPM). It's possible (and a good idea) to harden Linux computers with Secure Boot.
...
On an MBR disk, there's a gap just after the MBR and before the first system partition. This permits a boot loader to exist in that gap for the user to select a [partition](/files/) (and, naturally, an [operating system](/os/) on it).
...
With GPT systems, the first sector is blocked off to error out legacy systems that use it, then use LBA 1 to boot from the primary GPT header. In a Linux system, you can access the EFI system partition (ESP) at /sys/firmware/efi, with the [file](/files/) having a .efi extension.

## Features

Booting isn't always so straightforward. Instead, in an [enterprise](/enterprise/) situation you can use a remote-access boot command with a pre-boot execution environment (PXE):

1. The computer lets the [DHCP](/protocols/) server know that it's doing a PXE boot.
...
The operating system will use a variety of files, which will have different arrangements depending on [how the file system is designed](/files/).
