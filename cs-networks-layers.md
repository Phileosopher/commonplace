...
Computers talk to each other with "[protocols](cs-standards.md)", which keeps the computers from misunderstanding each other. It's an elaborate version of when people say "over" and "copy" on a walkie-talkie. Protocols let *many* computers all talk seamlessly on a large system.
...
- Twisted pair - tiny copper wires (usually 8) twisted into pairs, surrounded by a sheath, can be unshielded or shielded.
  ...
  - RJ11 is the standard old-timey telephone cable and only has 4 wires, RJ45 is what everyone thinks of when they say "ethernet cable". Ethernet has been a prevalent standard since the 1970s, mostly because they [publicly released their standards](legal-ip-floss.md) to the [IEEE](cs-standards.md).
...
- Wi-Fi - local radio wave transmission, uses [802.11 standard](cs-standards.md).
...
To speed up networks by avoiding all that useless data transfer, engineers designed the "switch", which is directing the data only to where it's supposed to go using a variety of [multiplexing protocols](cs-alu.md). One of those protocols is the "spanning tree protocol" (STP)
...
2. Most of the [cybersecurity concerns](cs-infosec.md) with wireless networks comes from the physics-based reality that a Wi-Fi signal can only exist as a network hub.

Unlike the other 6 layers, Layer 2 has two sub-layers:

1. MAC - media access control, which clarifies what device is being used.
   - The "MAC addresses" are built onto NICs by manufacturers, who have to request a block of them from the [IEEE](cs-standards.md).
...
There's a maximum transmission unit (MTU) size that determines how big frames can get, but if you have a situation like [virtualization](cs-distsys-vm.md) where there's technically nothing long-distance, you can use what's called "jumbo frames" to speed everything up.
...
- Carrier sense multiple access with collision detection/avoidance (CSMA/CD / CSMA/CA) is a system within the Ethernet [protocol](cs-standards.md) that either detects a frame collision or prevents it beforehand.
...
1. IPv4 was the standard for a long time (e.g., 192.168.0.1). It used 32 [bits](cs-alu.md): four octets of binary information (0-255, 4 times).
...
6. While there are tremendous opportunities of having special-use devices that operate as independent network assets, it created tremendous [cybersecurity risks](cs-infosec.md) as well.
...
[P2P](cs-distsys-p2p.md) is a relatively newer standard that involves a request and response model where *all* the computers are sending and receiving information simultaneously.

Depending on the protocol, the data will transfer differently:

- Simplex - one way only (e.g., [keyboard](cs-keyboard.md) or [screen](engineering-screen.md))
...
This mostly has to do with [character encoding](cs-keyboard.md) along with [encryption](encryption.md).

The PDU at the Presentation layer is the data formatted for delivery (if outgoing) or presentation (if incoming).

Often, the issues on the Presentation layer are tied to bad [operating system](cs-os.md) drivers or permissions issues involving the user's access level, and sometimes [malware](cs-infosec-cysec-malware.md).

## Layer 7: aplication

The Application layer is a bit of a weird catchall. It merges closely with [front-end development](cs-software.md), since it deals heavily with [how the user perceives the information](engineering-design.md).
...
Applications can be bandwidth-sensitive (e.g., a streaming movie) or elastic (e.g., email or a [web browser](cs-browser.md)). Faster bandwidth is always better, but not always necessary.
...
The line between Layer 7 and [most individual operating system elements](cs-os.md) mostly differentiates with whether data transfers from one device to another, though that can get *very* blurry due to [distributed systems](cs-distsys.md).
...
For [cybersecurity reasons,](cs-infosec.md) some information must be [encrypted](encryption.md) across the internet. While this is *usually* done on Layer 3, it can also be done on Layer 2 (such as L2TP).

Further, some protocols run across *multiple* layers. [Mobile](engineering-radio.md) carrier [protocols](cs-standards.md) run across L1, L2, and L3:
