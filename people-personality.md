...
It's difficult to intuitively [classify](logic.md) personality because psychologists are attempting to group the basis for how we make [decisions](mind-decisions.md). For this reason, most personality indicators and tests are vaguely close to reality but always miss certain elements. The only exception to this is the Big 5 personality indicator, which is the only one with any [scientific rigor](science.md) behind it.

- The Myers-Briggs Type Indicator (MBTI), for example:
  - Extraversion vs. Introversion is the same.
  - Sensing vs. Intuition maps to Neuroticism.
  - Thinking vs. Feeling maps to Openness to Experience mixed with Neuroticism.
  - Judging vs. Perceiving maps to Conscientiousness mixed with Agreeableness.
- Other tests are actually testing what we *do* more than what we *prefer* (e.g., DISC assessment, 5 Voices)
- This makes the entire personality assessment industry a well-[marketed](marketing.md) variation on fortunetelling and horoscopes.

Personality carves itself into [5 major classifications (with 2 subclasses)](people-personality-ocean.md) that conveniently break out into an OCEAN acronym. It typically represents as the [conflict](people-conflicts-inner.md) between two decisions that have [benefits](meaning.md) and [risks](safety.md), with our [soul's](humanity.md) decisions making the deciding factor that determines how we wish to continue from that point.
...
4. Release *everything* about yourself as a product of the environment that was given to you, minus what you have [control](mind-decisions.md) over right now. In practice, all aspects of [identity](people-identity.md) must come strictly from [decisions](mind-decisions.md) you still stand by.
...
APPLICATION: People who integrate their shadow can be terrifying for those who haven't. They have, in a way, transcended to another [frame](people-image-why.md) of existence where their [identity](people-identity.md) is only contained in who they are instead of the [environment](reality.md) they're in. This arrangement makes it *very* difficult to exercise [raw power](power-types.md) over them, and they can only be influenced via [persuasion](influence.md).
