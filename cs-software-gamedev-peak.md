...
Game development is all effective aspects of [software development](cs-software.md), but requires *many* [specialized](jobs-specialization.md) roles that would otherwise be optional or unnecessary in most other software domains:
...
- Programming roles that branch out into all standard software development, including [graphics](engineering-graphics.md), [animation](art.md), [AI](cs-ai.md), [servers](cs-distsys.md), physics, pipeline engineers, build engineers, testing, visual effects, databases, [language-specific](cs-langs.md) roles, and roles specific to game engines.
...
- [Quality assurance](cs-software-redesign.md) roles, especially play testing.
...
2. The [audio](engineering-audio.md) must be synchronized to the visual experience, so it has to be correctly designed, from the sound effects to the soundtrack.
3. Games require *immediate* feedback from the input peripherals (such as the [keyboard](cs-keyboard.md) or [mouse](cs-mouse.md), and now [VR](cs-xr.md)). While people can endure a 1-second delay in many other programs, 0.1 seconds is enough to make a janky game.
4. For many games, you need an elaborate [mathematical framework](cs-langs-algorithms.md) to keep all the visual and audio elements working in tandem. This is *much* more than most other programs.
5. Across the internet, the demand for rapid-response [networking](cs-networks.md) in many genres is a perpetual need, for *both* player input and [visual output](engineering-screen.md).
6. In multiplayer games that need computer players (e.g., a 2v2 game with 3 humans), the [AI](cs-ai.md) has to have enough logic to permit the player to feel like the "bot" is human-like.
7. [Graphics](engineering-graphics.md) technology is often driven by large-scale game developers, who build incredibly elaborate designs for their games that need ever-increasing [processing power](cs-cpu.md). This ranges from the character designs all the way to expansive and beautiful "skyboxes" and long-distance rendering to recreate extremely large worlds.
...
- Even through to today, vintage [game emulators](cs-distsys-vm.md) are notoriously difficult to build because the software must reproduce the individual clock cycles and constraints of the original hardware.

### Netcode

Across a network, games have extremely stringent requirements for input, and heavy requirements for output. From frame to frame across a [network](cs-networks.md), the game must match on *both* computers.

Online games are typically designed as "deterministic", which means two instances of the same program will create *precisely* the same outputs with the same inputs. Even random numbers can be [algorithmically](cs-langs-algorithms.md) derived deterministically. This gives the added convenience that a replay of a game can simply be the inputs mapped to timing inside a game, but also allows other games to reconstruct a match even with [network](cs-networks.md) latency.
...
- The only exception to this is specific game genres (e.g., visual novels, some [VR experiences](cs-xr.md)), but advancing the interface doesn't define as a "game" (i.e., there should be a capacity for [making decisions](mind-decisions.md)).
...
2. The game must be challenging enough: too much and the player will "ragequit", too little and the player will be bored. The challenge must come from the player's *interaction* with the game, not from [software errors](cs-software-redesign.md) or arbitrary game design that stretches out time.
