...
Usually, when people think of web development, they imagine web browsers. However, there are very specific aspects to browsers that specialize far more than simply the internet as a collective [network](/network/).

## Requests

Most of the internet's backbone for daily use is the HTTP protocol (hyper-text transfer protocol) and HTTPS (its secure version). Historically, it only sent HTML, but it now sends CSS, along with all sorts of [more advanced code](/langs/).
...
- GET - ask for information from a specified location (sent in the URL directly and therefore *not* [securely sent](/security/))
...
- HEAD - same as GET, but won't return any information, great for [testing](/software-redesign/)
...
- TRACE - performs a message loop-back test to test the path to the target resource, also great for [testing](/software-redesign/)
...
- 300s - the request is being redirected
  ...
  - **305 Use Proxy** since *only* a proxy has the information, used since HTTP/1.1 but many web browsers don't obey this code because it's a wide-open risk for [hacking](/hacking/)
...
- 400s - the client computer has an error
  ...
  - **401 Unauthorized** is similar to 403 but [authentication](/authentication/) wasn't provided or has failed
  ...
  - **451 Unavailable For Legal Reasons** because the server operator can't [legally permit it](https://notageni.us/legally-safe/) ([RFC 7725](https://datatracker.ietf.org/doc/rfc7725/))
- 500s - the host computer has an error
  ...
  - **511 Network Authentication Required** by the client to gain network access, typically from intercepting proxies that control access to the network (e.g., "captive portals" to require [agreement to Terms of Service](https://notageni.us/contracts/)) ([RFC 6585](https://datatracker.ietf.org/doc/rfc6585/))
...
These TLDs are authorized by [ICANN](https://www.icann.org/), a [nonprofit-in-name-only](https://notageni.us/mgmt-npo/) that issues those domains. Different organizations can buy them, then sell the subdomains at a fixed price. Some TLDs (like .edu or .gov) are only granted to specific authorities, but most of them are free-for-all first-come first-serve.
...
- There are many, *many* DNS records, but the bare-bones specifications mostly exist as [standards](/protocols/) in [RFC 1035](https://www.rfc-editor.org/rfc/rfc1035).
  - These records can be for specific resources, or "wildcard" across many possible situations.
- A - what [IPv4 address](/networks/) to look for.
- AAAA - what IPv6 address to look for, specified by [RFC 3596](https://www.rfc-editor.org/rfc/rfc3596).
- CAA - [Certification Authority](/authentication/) Authorization, specified by [RFC 6844](https://www.rfc-editor.org/rfc/rfc6844).
...
To access most websites, a computer will [access](/network/) several computers that sit potentially across the world:

1. Since it's rapid compared to anything else, the software first looks within cache [memory](/memory/) locally stored in its memory. Later, it'll also typically archive more information to that cache.
...
3. If the name server doesn't have the information, it'll ask the root server, (also known as the Global Top Level Domain Server, or GTLD) to find the name server with the relevant name (e.g., there's a GTLD for all .com domains).
   - There are also [alternative DNS roots](https://en.wikipedia.org/wiki/Alternative_DNS_root) as well, beyond the standard ones, which allow for [decentralized systems](/dist-sys/), [cryptocurrency](/blockchain/), or [nations the ability to micromanage their internet](/faang/).
...
5. The client accesses the DNS record held at a DNS resolver (e.g., CloudFlare at 1.1.1.1, Google at 8.8.8.8, Quad9 at 9.9.9.9).
   ...
   - It's not *too* difficult to make your own DNS resolver, but most DNS resolvers used by most websites are [gigantic FAANG corporations](/faang/).
...
9. Further, there are extra [security aspects](/security/), such as SSL/TLS and DNSSEC, that make [malicious hacking](/pentest/) more difficult.
...
1. Core DNS was made in the 1980s when the internet was smaller, so the 1990s saw DNS Security Extension (DNSSEC), which adds [encryption based on public key cryptography](/encryption/) to DNS standards.
...
3. DNS over HTTPS (DoH) and DNS over TLS (DoT) have [encrypted](/encryption/) the information to the DNS resolver, but the resolver can still see the site the client is trying to visit.
...
- To that end, many people who [scam](/social-engineering/), [hack](/pentest/), and [host pirated content](/torrent/) like using cheap/free, disposable domains, which may affect a domain as a [marketable](https://notageni.us/marketing/) address when legitimate organizations block the entire domain extension.
- Some domains (e.g., .de) will validate *before* registration is approved.
- Some domains require more information (e.g., .us requires submitting a connection to [your legal fiction](https://notageni.us/requirements/)).
...
2. Sort and process the information to create [database](/database/) associations between the keywords and "hyperlinks".
3. Run a [search algorithm](/algorithms/) whenever the user requests a keyword or phrase.
...
Most search engines are so powerful that they tend to "web scrape" entire websites as well. This is particularly egregious [in the case of extremely large groups](/cysec/).

## Search engine optimization

Since businesses want to make lots of money, there's an entire division of [marketing](https://notageni.us/marketing/) that specializes in nothing but "search engine optimization" (SEO). [It involves a *lot* of technical back-end](https://www.toptal.com/developers/webdevchecklist):
...
3. Add an [SSL certificate](/security/) and make sure it's sending secure information.
4. Add links all over the site that link to other parts of the site (~3-4 links every 1500 words).
5. Do absolutely *everything* you can to improve [web/app accessibility](/ux-ui/).
   - Make sure your [UI](/ux-ui/) is properly color-matched.
   ...
   - Use short URLs that don't have dates in them and are never more than 2 [subfolders](/files/) deep.
   - Set social media image, title, and description (e.g., Open Graph [protocol](/protocols/) for Facebook, viewport meta tag for mobile).
6. Do absolutely *everything* you can do to speed up the website, both for users and for web crawlers:
   - [Debug](/software-redesign/) all the code and delete unused code.
   - Compress and locally host all the images and videos.
   - Locally host your fonts.
   - Add keywords into the body of the page, as well as inside the title tag and [meta](/data/) description.
7. Test *everything* with [emulators](/vm/) and real hardware for all sorts of weird "edge cases".

Obviously, some less-than-legitimately-motivated people have wanted to exploit the situation. If you've ever clicked through to an odd webpage that says, "Here's how you can have the best bicycle tires that are tires for biking, of which bicycles are the best you can bicycle to and from work for bicycling", you're looking at SEO gone horrifically wrong. Thankfully, the engineers behind the search algorithms usually punish sites that [hack their algorithm](/hacking/) in the long-term, so we can still achieve high-quality results.

## Web browser features

Most typical users interact with the internet through a "web browser", which is software specifically designed to handle a wide variety of internet content including pictures, [videos](/screen/), and [games](/game-dev/).

While some web browsers emphasize [cybersecurity](/cysec/) and others emphasize speed, they almost always do just about the same thing: give a safe and fast web-browsing experience that also *doesn't* download bloated, broken or [malicious](/malware/) code.

Most information displayed on a web browser is "HyperText Markup Language" (HTML), which is more of a markup language than a proper [scripting language](/langs/). It pairs with "Cascading Style Sheets" (CSS) that give visual style, as well as a scripting language (which is often JavaScript).
...
- However, stored cookies can also lead to [privacy issues](/cysec/), especially when third-parties (like advertising companies) can track behavior *across* websites or when [hackers](/hacking/) track where you're browsing.

The Tor [protocol](/protocols/) and its browser adds layers of anonymity to the web-browsing experience, which means more layers of complexity.

## Browser/page UX

While the [user experience](/ux-ui/) of mobile devices often separates the experience of internet-browsing into a wide variety of "apps", most of those apps are simply various specialized web browsers inside the [operating system](/os/) (e.g., Electron app).

Differently sized screens and inputs are *not* trivial issues! The wide variety of [screen implementations](/screen/) means the user could be using a [mouse](/mouse/) and [keyboard](/keyboard/) on an office machine, their cell phone or tablet with their fingers, or an interactive [VR headset](/vr/). To make it simpler, developers sidestep the pixel measurement in lieu of a root em (or "rem") measurement based on a basic font size (typically 16 pixels).

At one time, near the year 2000, almost every internet-enabled computer was working on about a 1024×768 [screen](/screen/). Now, they can range from 480×320 through to 3840×1080 (and growing), with all sorts of odd rectangular shapes (with [VR](/vr/) and [driverless automotives](/cars/) adding even more variety). This adds a layer of challenge to good web design, but has a few simple tricks:
...
To maintain a workable [UX](/ux-ui/) across the wide variety of screens a web browser may use, front-end web developers use "media queries", which are instructions to change or add/remove elements depending on the size of the screen. This is called "responsive web design", which you can see simply by going to a web page with media query code (like this one) and scrolling in and out (either by pinch-zooming or CTRL and +/-).

## Visual elements

Visual web design is [an art form of its own](https://notageni.us/design/). Besides all the conventions of [user experience](/ux-ui/), the site must be SEO-friendly (i.e., low-enough memory to quickly transfer across the internet). This means all aspects of programming [web graphics](/graphics/) is a Zen art: enough to get the point across with as little data transfer and distraction as possible.

One of the easiest ways to design motion into websites is by using a CSS feature called "keyframes". Instead of getting in the weeds with [graphics](/graphics/), developers can indicate multiple states with percentages, then the language has enough logical power to deduce the shifts:
...
It's worth noting that CSS is not the *only* way to design graphics. For certain use cases, other 2-dimensional graphical representations like Canvas are better options. And, for 3-dimensional graphics, [shaved-down game engines](/graphics/) like Unity and OpenGL work better.

While it's barely detectable to most people, web browser pages have a "favicon" logo sitting in the web browser tab (or in the Task Manager tray in the [operating system](/os/)). This little square logo ranges from 16 to 64 pixels wide and shows up in a *lot* of places in a web browser including web searches, history, and dropdown menus.
...
2. The data may become more complex than text and allow [images](/camera/), [videos](/camera/), and [audio recordings](/speakers-mic/).
...
7. Each element of data might give the user freedom to constrain [permissions](/authentication/) for which users (or unidentified computer users) can access the information.
...
Further, the entire social network can be [distributed](/dist-sys/) as-needed, with the extreme version of it being *completely* decentralized (such as a [blockchain implementation](/blockchain/)).
...
In fact, Electron apps are effectively working with all the inefficiencies of a web browser. While it's *great* at building a quick cross-platform solution, the greatest [efficiencies](/software-redesign/) will come in rewriting the software for each of those particular [operating systems](/os/).
