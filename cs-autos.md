...
"Autonomous vehicles" (AVs) are effectively [robots](cs-robotics.md) designed as cars on the road, which means all the risks of robotics apply, except at a *much* faster speed that can potentially kill people:
...
- While [AI](cs-ai.md) can generate *some* intuition, [debugging](cs-software-redesign.md) it is an absolute nightmare.
...
- Detecting [sound-based](engineering-audio.md) matters with [microphones](engineering-audio.md) from every angle, such as car horns or collision sounds.
- Using incessant [GPS tracking](logistics-navigation.md) to specify the location and feed a [learning model](cs-ai-ml.md) for all the *other* AVs, as well as complying with regulatory geo-fence requirements.
...
2. Information from the input travels into a [CPU](cs-cpu.md) as an input number (e.g., accelerator, brake, steering).
...
Further, most cars are also equipped with EDRs (electronic data recorders), so the information is already outputted as a computer-friendly format, and many parts of it are [standardized](cs-standards.md):

- The OBD-II sensor can have that data requested on command with a cheap reader.
- You can gather driving metrics by plugging in an OBD-II device that connects via a [wireless network](cs-networks.md).
...
Even without *any* autonomy, the CAN bus is supposed to be "air-gapped", but all the extra features for the sake of convenience (e.g., headlights accessible via a phone app) have made automotives some of the most [insecure](cs-infosec.md) hardware on the planet.
