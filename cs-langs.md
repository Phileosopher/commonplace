...
2. Manipulating it, typically with a coded [algorithm](cs-langs-algorithms.md) stored in [memory](cs-memory.md)
3. Outputting it, storing it in [memory](cs-memory.md), or both (and sometimes manipulating the coded algorithm in #2!)
...
Computers store both their programs *and* their results in [memory](cs-memory.md). While this creates a tremendously versatile system, it also can be confusing because nothing else in reality conforms to this approach (imagine using construction equipment as part of a house's design, for example).
...
2. Where in the [memory](cs-memory.md) will it do those things?
...
At its core, programming *could* be machine code: just a bunch of 0s and 1s. But, that's tiresome, and hard to read, so they made [assembly code](cs-langs-specific-assembly.md) that's easier to read.
...
So, programmers made (and make) high-level languages. These languages let you type 1 command that does dozens, hundreds, or thousands of "lines" of assembly code. When the programmer is done and runs it through a "[compiler](cs-langs-compilers.md)" or interpreter, it converts it to assembly code, though it may use an "interpreter" that converts it to assembly code as the computer reads each line.
...
Frequently, [high-level languages](cs-langs.md) can define all sorts of reusable types, classes, and variables. Plus, high-level languages can include "libraries" and "frameworks" to make them even *more* useful.
...
To work with high-level language, programmers usually use an "integrated development environment", or [IDE](cs-software-ide.md).
...
This can range from [console commands](cs-cli.md), [the mouse or touchscreen](cs-mouse.md), and can even include [AI prompts](cs-ai.md)!
...
There are *many* languages, and they can be broadly classified by how they're used, but also the pathway between how written code converts to machine code (generally, [compiled](cs-langs-compilers.md) versus interpreted) and how [primitives](https://trendless.tech/primitives/) are constructed into more advanced components.

### Assembly code

[Assembly](cs-langs-specific-assembly.md) is a low-level language compiled by an assembler.

They're often the intermediate stage between a higher-level language and machine code, and tend to be proprietary to the [processor](cs-cpu.md) (e.g., AMD's assembly code is different from Intel's).

### Scripting languages

Many times, you'll want to run a [console command](cs-cli.md), and do it based on certain conditions. Most operating systems come with pre-built scripting languages:

- [Windows](cs-os-windows.md) has PowerShell, though it once used batch files.
- [Unix](cs-os-unix.md) generally uses bash, but has a wide variety of other ones (as is the nature of [open-source software](legal-ip-floss.md)).
...
They typically are interpreted, and don't need [compilation](cs-langs-compilers.md), and are comparatively simpler than other languages for the computer to understand.
