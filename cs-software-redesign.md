...
Often, logic errors earlier in the code's execution will create runtime errors later. This is *extremely* common in complicated computer activities, such as [operating systems](cs-os.md) or [high-graphics](engineering-graphics.md) [games](cs-software-gamedev.md). Shrewd developers hide the error messages while the brilliant ones fix them.

Another easy way to avoid logic errors is to practice [good coding habits](programming-tech.md).

There are a *ton* of simple rookie mistakes with [file/folder management](cs-files.md) that create logic errors:
...
- Failing to reference the correct folder/path ("./docs/image.jpg" is [*not* the same](cs-cli.md) as "../docs/image.jpg")
...
**Platform errors** are issues with the code once it hits the real world: [network](cs-networks.md) issues, [hard drive](cs-memory.md) failure, invalid [file permissions](cs-infosec.md), etc.
...
Computers give a *lot* of information about runtime errors. They frequently store many log files *all over the [file system](cs-os.md)*. In fact, they can sometimes store so many "plaintext" error log files that it often creates [memory](cs-memory.md) bloat.
...
The only way to thoroughly be prepared for publishing software is with an "end-to-end test", where you run *everything* live, and at the scale you expect to see. This is often difficult to set up individually, but [enterprise-grade](cs-distsys-enterprise.md) development practically needs it.
...
- Break apart the tasks into modules that each do 1 thing. This becomes the beginnings of making [libraries and APIs](cs-langs.md).
