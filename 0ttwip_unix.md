...
However, the demarcation between Linux/Windows/Apple isn't precise. Each operating system borrows *heavily* from the others, especially in the domain of [UX](/ux-ui/). In particular, GNU's system expands far into the realm of Windows-based operating systems, so it's not really an exclusive concept as much as [a FLOSS ideal](/floss/).

## History

Unix was an [operating system](/os/) made by AT&T in 1969. However, AT&T was monpolizing power at the time (like [today's FAANG](/faang/)), so antitrust litigation prevented it from entering the computer industry. The company was forced to release the source code for free to anyone who asked. Many educational institutions (such as Berkeley) and businesses used Unix.
...
In 1983, Richard Stallman created the GNU Project, which aimed to create an entire Unix-compatible software system made of nothing but [free software](/floss/). GNU stands for "GNU is Not Unix", which is a [programmer joke](/geeky-jokes/) about [recursions](/prog-basics/), and he wrote [the GNU Manifesto](https://www.gnu.org/gnu/manifesto) to rally support for a completely free operating system.

Many of the programs for a fully-functioning Unix "clone" were completed by the early 1990s ([libraries](/prog-features/), [compilers](/compilers/), [text editors](/ide/), a [command-line shell](/consoles/), and a [windowing system](/graphics/)). However, the lower-level elements like device drivers, daemons, and the kernel were incomplete.
...
One of the draws of Linux is that it's completely free compared to Unix and other proprietary systems like [Windows](/windows/) or Apple. That freedom gives a few built-in features:

- People tend to feel generally safer with it, since the operating system is a community-led project.
- When a distro is popular, [bugs](/software-redesign/) are quickly dealt with, since there are many eyes on that project.
- If someone develops anything driven by that back-end, there's little to no risk of [coercive behavior](/floss/) later on.

The culture of Unix and Linux holds together with a philosophical framework driven around [the varying ideas that constitute software freedom](/floss/).
...
- A few are for [PenTesting](/pentest/), like [Kali Linux](https://www.kali.org/) or [Pentoo](https://pentoo.ch/).
- A handful make Linux into a portable media center, such as [Kodi](https://kodi.tv/).
- A few others were made for [computer security](/security/), either with [virtualization](/vm/) ([Qubes](https://www.qubes-os.org/)) or [anonymity](/cysec/) ([Tails](https://tails.boum.org/))
...
- Many of them honor the GNU philosophy directly (like [Devuan](https://www.devuan.org/)), which means any non-[FLOSS](/floss/) software won't work with the system.
- Others are meant for [enterprise-grade work](/enterprise/) like [Apache HTTP Server](https://httpd.apache.org/), [SUSE Linux](https://www.suse.com/), or [Red Hat Enterprise Linux (RHEL)](https://www.redhat.com/en/technologies/linux-platforms/enterprise-linux).
- Some are designed for specific hardware:
  - [libreCMC](https://librecmc.org/) for an all-in-one embedded OS
  - [Raspbian](https://www.raspbian.org/) for the [Raspberry Pi](/pi-arduino/)
  - [Android](https://www.android.com/) for [Google](/faang/)-based phones
...
- If you have the patience and time, as well as the [marketing](http://notageni.us/marketing/) acumen to [gain adoption](https://gainedin.site/trends/), you could make your *own* GNU/Linux distro.
...
OS frameworks that originated from Unix all have a similar-enough experience to make the user [feel at home](/ux-ui/) when swapping back-and-forth between them, including using many of the same [commands](/consoles/). To that end, the broad catchall term is "Unix-likes".

Apple also uses a Unix framework, for both MacOS and iOS, but its key difference is that it's a hybrid of a ["monolithic kernel" and "microkernel"](/os/), while Linux is *strictly* a monolithic kernel.
...
- FreeBSD components
- IOKit, the C++ [API](/software-design/) for writing drivers

## Usage

Linux is lightweight, so it's excellent for low-profile computers, and is even convenient to run off a flash drive.

However, unlike [Windows](/windows/), Linux has *also* become more consistently reliable.

The largest failure within Linux is tied to its [UX](/ux-ui/). For the most part, its [GUI](/graphics/) commands convert to [console commands](/console/), meaning any sufficient experience with Linux often requires accessing a terminal. This creates [a learning curve beyond the aptitude of some people](https://adequate.life/computers/).
