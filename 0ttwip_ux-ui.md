...
All the [general rules of good design](https://notageni.us/design/) apply, but computers take a divergence in a few key areas.

## Rules

The speed of successful [tech trends](/trends/) mixed with the relatively young age of most [tech innovators](https://gainedin.site/trends/) tend to see the industry *constantly* break the rules of UX.
...
- Make operations difficult to understand.
  ...
  - Make all the information available in [the manual](/documentation/), instead of in the interface.
- Be impolite.
  - Treat errors and near-errors as completely invalid.
  - Make errors seem or be very dangerous, as if they're [breaches of contract](https://notageni.us/contracts/).
  - Permit 1 error to destroy valuable work.

Users can only draw from their [environment](https://gainedin.site/reality/) (observations) or [memory](https://gainedin.site/imagination/) (expectations), so [computer design](https://trendless.tech/ux-ui/) frequently borrows from the world around us:
...
We draw [patterns](https://gainedin.site/symbols/) that connect different aspects of the world around us ("skeuomorphism"), so it's *absolutely* critical that technology reproduce the feedback mechanisms in other existing technology or in nature.
...
- Make sure the presented information is using any tracked [location data](/gps/), and make it location-agnostic if there's a [VPN](/cysec/) or no location. Generally, there's still usually an *approximate* nation or region to derive data from.
- If there is *any* information about the [operating system](/os/), assume the user is using that particular OS. Either way, give a simple selection for *all* available OSes.
- If the user has provided information, use it to tailor all relevant [algorithms](/algorithms/) of the software (e.g., their genre preference should define *all* the content).
...
- Create designs that direct the user's incentive toward something else that is *not* the computer. We are [wired to need human connection](https://gainedin.site/universals/), and computer-assisted [interaction](https://gainedin.site/conversation/) is typically *less* [meaningful](https://gainedin.site/meaning/) than in-person.

## Communication

Typically, the nature of tech trends means most of the styles communicates from the sci-fi/fantasy of the time to imply the technology is "futuristic" by that decade's [fashions](https://gainedin.site/trends/).
...
[Software engineers](/prog/), by the nature of their trade, do not need great [communication skills](https://gainedin.site/conversation/) and often have limited [people skills](https://adequate.life/people-2/), so many computer design elements have been, from their inception, fundamentally obtuse.

This is almost standard in tech, and never seems to [trend](/trends/) otherwise. Most software is either bloated or under-communicates, depending on the psychological idiosyncrasies of the software engineer and design team (if any).

[Cybersecurity](/cysec/) design, by contrast, is simply the *reverse* of good design: instead of communicating what a user can do, the designer is obscuring and *hiding* things instead.

Of course, this means many possibilities for growth within computers. Many more things frustrate users in computers than anywhere else, so there are more possible places where [taking a risk](/entrepreneur/) may yield a positive result.
...
With software, it's not difficult to include help files and browser-based [hyperlinks](/web-dev/) to lead the user to what they need. With hardware, it's worth including both simplified and advanced documentation for different levels of users, or to at least maintain a permanent archive hyperlink built onto the package.
...
Forcing functions within the domain of computers is *way* more dangerous than any other domain. While people may [hack](/hacking/) their fridge or car to do what they want, tech hackers are *very* intelligent and *really* hate forcing functions, especially lock-outs. Many of them will try to break a lock-out on principle alone.
...
There are *many* elements to consider for interacting with a computer. Most of them are automatically designed into most [operating systems](/os/) or [web browsers](/web-dev/), but anyone with patience to learn can often tweak, add, or remove them.
...
- All measurements should be [mathematically](https://notageni.us/math/) related to each other.
...
A user needs *much* more information from computers than most products to make their [decisions](https://gainedin.site/decisions/):
...
- However, the farthest edges of [screens](/screen/) adds additional accessibility. The *very* edge of the screen is effortless to navigate to because it captures *all* distances where the selection goes beyond the screen (e.g., thumb touching the edge of a tablet). Those regions are called "magic pixels" from how convenient they are.

The buttons and selections should be seamless on *all* varieties of input peripherals the user may use:

- [Mouse-only](/mouse/) interfaces don't work well with swipe-based interaction, touchscreens don't have any sort of mouse-over function, and [audio-only](/speakers-mic/) can be *very* challenging to give prompt feedback without the user interrupting.
...
- If an input has any rules, explicitly communicate them nearby without any extra required input before the user enters the information (e.g., [password](/authentication/) rules, character limits).
...
- The software must ask for [permissions](/authentication/) relative to both when it is absolutely necessary *and* proportional to the user's trust of the software.
...
- Be careful with toggle switches (i.e., sliding checkboxes), since they give poor feedback compared to checkboxes (since they're pulling from [network](/networks/) information instead of sending that information *after* the user submits it).
...
- Avoid the browser's default file upload feature, since it varies wildly on the [browser](/web-dev/).
- For physical things with a relatively analog control scheme (e.g., 1 to 100), physical knobs and dials are almost universally superior to buttons on a screen. This, however, does add extra work for [project management](https://notageni.us/mgmt-projects/).
...
- Present most typical media in as downsampled a format as possible to save on [bandwidth](/networks/) and loading.
- "Media queries" that resize elements appropriately relative to the [screen size](/screen/) or other elements.
- Clear text or other types of media as a fallback if that media can't present itself (e.g., [sound](/speakers-mic/) has been muted, poor internet connection).
...
Since most interaction with a computer is [screen-based](/screen/), color is *vitally* important for communicating information on the [front-end](/graphics/) and [across the internet](/web-dev/), more than most other non-computer design:
...
Shadows are dark edges around the element to make the interface feel three-dimensional. They tend to vacillate with [social fashions](https://gainedin.site/trends/) with an attempt to make everything look three-dimensional:
...
Most UX design revolves around [web development](/web-dev/), so make sure the hyperlink is accessible:
...
[Software development](/software-design/) has a unique, nasty problem:

1. First, the software must be designed, then [tested a *lot*](/software-redesign/) to ensure it actually works.
2. Then, the software must be tested to ensure that people *beyond* the designer are capable to use it.

The implication of this is that the [project management lifecycle](/mgmt-projects/) in a large organization will likely be *twice* what the manager is probably thinking.
...
- The people who design software are human beings who [may not want the users' best interests](/faang/), and have reason to employ "dark patterns" to make some elements difficult to access (e.g., unsubscribe, delete account, remove connection).
- [Social trends](https://gainedin.site/trends/) can redefine elements, so most websites are *constantly* changing. Some elements become archaic, new elements become the standard, and some elements can become purely offensive.
...
- The only way to keep an inherently complex product like software or a computer simple is to *constantly* investigate how to merge or remove obsolete features. Just make sure you explicitly document those changes, and allow people to roll back [those updates](/software-redesign/) if they need.
...
Generally, large-scale changes can force an immobilized design experience. Back when [Windows 95](/windows/) dominated the OS market, people could tweak their wallpaper, colors, and sound effects, but most granular control is constrained in most mainstream [OS's](/os/). Having the freedom to keep a customizable interface would require exponentially more work, or the quality would suffer.

At least, theoretically. Customizable elements heavily contribute to a user's feeling that the interface is "theirs", and gives the users a more [meaningful](https://gainedin.site/meaning/) experience, even while it's more awkward.
...
- Requiring the user install extra software or accept extra [permissions](/authentication/) that are convenient for the developer, but often involve [tracking data that the user may not want tracked](/faang/).
- Removing any features that the users had become familiar with.
- Messages and prompts with excessive wording and explanation, instead of providing [documentation](/documentation/) for people who wish to read more.
...
- They'll be *much* quicker to adopt trends, and the speed of adoption determines how fast it'll get abused. It can lead to absolutely absurd interfaces until it's [patched later](/software-maintenance/).
...
- Asking for [permissions](/authentication/) or for the user to download software is highly convenient for the developer, even while it's annoying for the user, and that selfishness bleeds into the software experience.
...
- If it's consumer-facing (e.g., [web design](/web-dev/)), the push for simplicity and ease-of-use will make arcane systems hiding behind deceptively simple visual elements.
- If it's more technical or organizationally facing, the constant [feature bloat](/software-maintenance/) will make the interface cluttered with all the extra edge cases present for the user.
...
1. We must abide by [existing convention](https://gainedin.site/habits/) to learn how to operate something new.
2. Simplicity is critical for people to easily, quickly [understand](understanding.md) information that helps them make [decisions](https://gainedin.site/decisions/).
...
In technology, the "slow" element is the user's understanding. More than anything else, the slowest part of any designed computer that isn't a [large-scale distributed system](/dist-sys/) is the user.
...
Computers can magnify [addiction](https://gainedin.site/addiction/) more than just about anything else, mostly because a computer's design permits room for *many* dark patterns without the user's awareness:
...
- Hiding a [contractual agreement](https://notageni.us/contracts/) behind a checkbox that has terms the user would certainly *not* agree to if they read it.
- A "download" button that's near the *actual* download button that typically contains [a virus](/cysec/).
...
Shame is a [manipulation tactic](https://gainedin.site/power-types/) that tries to coerce users to do something. In software, this can be magnified because the software was *designed* that way, meaning it's scaled with the number of users:
...
Sometimes, in the interests of [marketing numbers](https://notageni.us/marketing/), some UI decisions will be *terrible*:

- Email popups that confront the user 3 seconds into opening the website, since [A/B testing](https://notageni.us/design/) showed it gained 10% more subscribers (while not really considering the lead-to-sales ratio).
- Moving around highly established design elements, since marketing [fashions](https://gainedin.site/trends/) dictated it was trendy and the key demographic would like it.
...
- Copious [advertising](https://notageni.us/marketing/) that promotes an unrelated product as part of the software experience.
...
- Adding [advertisements](https://notageni.us/marketing/) in between users' content, with [algorithms](/algorithms/) that tailor those ads to be as associated as possible to what the user may want to have.
...
- Artificial notifications with no relevance that still provoke the [habit](https://gainedin.site/habits/) of checking and removing them.
...
Except for casinos, [game development](/game-dev/) contains some of the most addiction-inducing design possible. Games use goals and rewards to create an [operant conditioning chamber](https://en.wikipedia.org/wiki/Operant_conditioning_chamber) that leaves people *constantly* emulating the sense of reward that comes from [succeeding at life](https://adequate.life/), but within their small ecosystem:
...
- Provide *just* enough incentive to keep playing the "free" game, while constantly [advertising](https://notageni.us/marketing/) the paid game.
