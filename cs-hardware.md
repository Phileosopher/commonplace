...
All computers are simply electronic math machines, built up from [logic](logic.md) using two-based numbering and various forms of [programming](cs-langs.md) to accomplish specific purposes.
...
Across the years, computer "abstractions" typically stay about the same while its "implementations" [trend](people-trends.md) insanely fast through various technologies:
...
- In decades, it may be something else, such as [quantum superstates](science-physics-quantum.md) or [DNA](cs-biological.md).
...
In practice, this means a [hacker](mind-creativity-hacking.md) can cobble together just about anything that's conductive and attach it to anything. Except for circuitry that's too small to work with (e.g., cell phones), most low-grade electronics are very accessible (such as the [Raspberry Pi](cs-embedded.md)).

The design of a general purpose computer has [specific elements as well for each component](cs-hardware-general.md).

## Design: Software

Except for [logic gate primitives](cs-alu.md), anything can be assembled as software *on* logic gates, and further hardware design simply speeds up how fast the computer will perform.

In simple terms, computers are vast collections of [logic gates](cs-alu.md) that build into ever-more complicated things. When combined, working with them is extremely trivial.

Designers are *constantly* improving computers, but the gains in improvement are starting to become incremental (similar to the [autos](autos.md) that came before it). Even though they can become much faster, they're not really adding much to the experience anymore for a typical user, and even a low-tier desktop computer is more than enough for most non-[gaming](cs-software-gamedev.md)/design computer needs until [VR headsets](cs-xr.md) or [AI](cs-ai.md) become a popular trend to everyday people.
...
The [memory](cs-memory.md) holds "programs" that tell the processor what to do.
...
The [processor](cs-cpu.md) is where the magic happens. By using a set of *very* clear rules, a processor can convert the input data in endless ways.

A processor cycles the information between a control unit and [math/logic unit](cs-alu.md).
...
- It may output to [audio](engineering-audio.md) or activate lights.
- It might store it to the hard drive for long-term storage between power cycles.
- It may send the information [across an electrical cable](cs-networks.md) to be *another* computer's input.
...
- To ensure the data is safe, the computers are typically on a [RAID array](cs-memory.md).
- Several processors run inside the device at once, just in case one goes out.

### Lamecomputers

On the *extremely* cheap end, some general-purpose computers like the [Raspberry Pi and Arduino](cs-embedded.md) are designed as "single board computers". They're meant to be so affordable that you can give it to children (~$40).

In fact, if you're tenacious enough, you can even [build a computer in your own garage](http://sam.zeloof.xyz/first-ic/).

Also, like stated above, many computers can be specific-purpose. NAS (network attached storage), for example, is just a normal computer that does nothing but send information back-and-forth to other computers on a [network](cs-networks.md). You can even make one with a [Raspberry Pi](cs-embedded.md)!
...
1. Dust creates a blanket-like effect on the parts, which keeps heat in. Left unchecked, it can melt the computer, *especially* the [CPU](cs-cpu.md).
...
Computers *always* need updating. It can be the instructions for input/output devices ("drivers"), instructions for how the system that keeps it operating ("[operating system](cs-os.md)") or specific instructions for software.
