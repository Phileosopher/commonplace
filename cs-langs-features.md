...
One noteworthy reality is that there's no hard "box" around any of the languages. For example, someone *could* make a computer game with CSS and HTML, but they'd make better use of their time and effort to simply learn JavaScript or C. But, some people [do it for fun](cs-fun.md) or are insanely stubborn.

Strings are particularly tricky. To accommodate the fact that any [Unicode character](cs-keyboard.md) can be outputted, there are specific lines to text to preserve that output within a line of code that would otherwise express that text (e.g., "35%" becomes "35\%"). The input text is called a "string literal", which creates the "string value".

## Interpreted vs compiled

To understand this, [it requires understanding how compilers work](cs-langs-compilers.md).
...
All data comes in [various types](data.md). This may need to change. Some languages like JS automatically infer the change and can be forgiving, but others like [C#](cs-langs-specific-c#.md) and [C++](cs-langs-specific-cpp.md) need explicit retyping of data (static typing).
...
Beyond [debugging](cs-software-redesign.md) and writing code, a programmer spends a *lot* of time researching to find library packages that make their life easier.
...
- If you poke around in your [files](cs-files.md), you'll notice that you can have "Documents/Family/mom.jpg" as well as "Documents/September/mom.jpg" and "Documents/mom.jpg" at the same time.
...
Programmers tell the computer to grab a piece of [memory](cs-memory.md) and call it whatever they want (within reason):
...
An operating system can organize the [memory](cs-memory.md) to make referencing variables easier, programmers don't usually need to know about memory management when they're starting out. They just need to know what variables they're using and what's in the boxes.
...
Each computer language (its "syntax") is unique. Thus, while they're *all* [logical](cs-alu.md), they each structure that logic a little differently. Not following syntax rules makes the computer spit out errors, and is a huge part of [debugging](cs-software-redesign.md).
...
To prevent the high-level language from automatically changing the data type, you can use "===", "is" or some other variation depending on the [programming language](cs-langs.md):
...
To make the comments easier to distinguish, the [IDE](cs-software-ide.md) usually turns the comments gray.
...
A programmer who is [debugging](cs-software-redesign.md) that abomination you made later, however, will kill you. So, to avoid an untimely cessation of a primary organ, programmers agree on naming conventions.
...
Once [conditional statements](cs-langs-conditional.md) are established, the information can become more complex.
