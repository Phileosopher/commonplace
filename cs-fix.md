...
- Registry cleaners
  ...
  - Modifying the registry can permanently damage the [OS](cs-os.md), even with Windows Update.
...
1. Run an [antivirus scan](cs-infosec-cysec-malware.md), just to confirm.
2. Check for any running tasks, and sort by [CPU](cs-cpu.md), [memory](cs-memory.md), and [network](cs-networks.md) use to find what's using them the most.
...
11. Swap out your [web browser](cs-browser.md) for a lower-profile one that consumes less memory/CPU.
...
13. If you need a new motherboard for better parts, get a new computer for your purposes.
    - Use the old computer for a dedicated [side-project](cs-embedded.md) that won't see heavy use.

## Networked

Typically, it helps to triple-check that *every* [networked](cs-networks.md) component is offline, and preferably unplugged.
...
- Often, the [programmer](cs-software.md) will make code that worked for the situation (e.g., check ports 1 and 2 on a two-port switch), and it wasn't updated for a later hardware release (e.g., check all available ports).
...
One of the fortunate aspects about most computer troubleshooting (with the important exception of anything involving [AI](cs-ai.md)) is that the system is *highly* fine-tuned, meaning that it's not likely that more than one thing broke at once.
...
- Check that everything has been [rebooted](cs-boot.md).
...
- For software, check the [version numbers](cs-software-versionctrl.md) to be sure everything is up-to-date.
  - For [enterprise](cs-distsys.md) software, examine if there *were* any updates corresponding to the outage.
- Make sure the configurations are set correctly.
- Check the [network](cs-networks.md) to be sure it's working correctly.
...
- POST card for [boot](cs-boot.md) issues
- Loopback plug for [network](cs-networks.md) diagnosis
...
- It *can* be an upgrade if the situation permits (e.g., [keyboard](cs-keyboard.md), [mouse](cs-mouse.md)), but make sure it's compatible before getting it.
...
- Updates are generally not good to roll out, but software updates will frequently overwrite hundreds, maybe *thousands* of references, and you may need to [debug](cs-software-redesign.md) new updates and features *on top of* addressing your current problem.
...
- Sometimes, you'll simply have to hack the solution by ripping out the data yourself, then find the [protocol](cs-standards.md) that translates the raw data into a usable format.
