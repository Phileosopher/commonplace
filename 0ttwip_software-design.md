...
- "Front-end" development is making code to build out a [user experience](/ux-ui/), often on the [internet](/web-dev/).
- "Back-end" development is working heavily with [databases](/database/) and [algorithms](/algorithms/), typically with lots of [math](/math/).
...
- Understand [math concepts](/math/) (mostly the basics, since code is *always* working with math).
- [Creativity and imagination](https://adequate.life/creativity/) to see the product and [how the user will interact with it](/ux-ui/).
...
2. "Idiom" - an [algorithmic](/algorithms/) structure that the code builds into for most tasks.
...
- It takes time to plan and think through things, and most of that planning is part of a [creative process](https://adequate.life/creativity/) with a [logic-based](https://gainedin.site/logic/) implementation.
- Frequently, the implementation isn't always easy to put in place. This is especially true in [large](https://gainedin.site/groups-large/) or [badly managed](/your-job-sucks/) organizations, for a wide variety of reasons.
...
- A [version control](/version-control/) system (such as Git).
- An [IDE](/ide/) that highlights code and helps [debug](/software-redesign/), among many other things.
...
- A [cloud platform](/cloud/) to host apps and "infrastructure" for "end users".
...
6. Guess and start working now with what you can. With technology, it's almost always better to start and edit the results later than to wait until you know for sure, though [that's not always true](/exit-strategy/).
7. [Memory](/memory/) works in a hierarchy, and it's either fast or plentiful. While programmers want fast, cheap, and large memory, they'll need to trade off one of them. The capacity of various speeds determines the size of solvable problems. While the "cache" is a great workaround for memory limits, there are *always* hard tradeoffs between fast and large.
8. Computers need to be dependable, so make the systems "redundant" with multiple copies of everything. Use ECC, store results in different memory registers than inputs, have the software backup user data often, and verify anything downloaded with [checksums](/encryption/).

For each function, a programmer should design it first:

1. Issue analysis - identify the information that must be represented, and how the selected [programming language](/langs/) will represent it.
2. Data definitions - define what data will go through it, and make a good variety of sample input [data](/data/).
...
7. Testing - use the examples as tests and make sure the function passes all of them, which helps find all [errors](/software-redesign/).
8. Document - after making the function, create [documentation](/documentation/) to clarify how to use the function, using sample data to give examples.
...
To make less painful [errors](/software-redesign/) (especially logic errors), good software development tests the code incrementally:
