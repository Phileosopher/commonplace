...
Like [cars](autos.md), understanding how to operate computers is generally easier than understanding [how they work](cs-cpu.md).
...
- Most people connect advanced [video game design](cs-software-gamedev.md), [web design](cs-sofware-webdev.md), or [database development](database.md) to the concept of programming.
...
- Tapping or clicking something on a screen often runs the same command as if it were typed: a user can navigate to "C:\Program Files\WindowsApps", then run "calc.exe" with a [console command](cs-cli.md).
- Alternately, that user can use the mouse to select "Calculator", which will run that same "calc.exe" program.
- There are technically about 4-5 ways to do just about anything on a computer. If you're willing to learn [keyboard shortcuts](cs-keyboard-shortcuts.md), you'll become dramatically faster and [more productive](success-4_routine.md).
...
Even tech-savvy people sometimes get a virus or are [hacked](cs-infosec-pentest.md), especially when the virus spreads on a secured network.
...
- Numerous free online [cloud backup](cs-distsys-cloud.md) services (e.g., Dropbox) can help you frequently, routinely keep backups of your user files (and occasionally your program files as well).
