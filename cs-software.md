...
- "Front-end" development is making code to build out a [user experience](engineering-design.md), often on the [internet](cs-sofware-webdev.md).
  - Front-end work is often very visible, and the finished product is very clean.
- "Back-end" development is working heavily with [databases](database.md) and [algorithms](cs-langs-algorithms.md), typically with lots of [math](math-cs.md).
  - Back-end work often culminates in nothing more than an [API](cs-langs-features.md).

Categorically, there are three major types of software:

1. [Operating systems](cs-os.md) that make the computer function
...
2. "Idiom" - an [algorithmic](cs-langs-algorithms.md) structure that the code builds into for most tasks.
...
- A [version control](cs-software-versionctrl.md) system (such as Git).
- An [IDE](cs-software-ide.md) that highlights code and helps [debug](cs-software-redesign.md), among many other things.
...
- A [cloud platform](cs-distsys-cloud.md) to host apps and "infrastructure" for "end users".
...
7. [Memory](cs-memory.md) works in a hierarchy, and it's either fast or plentiful. While programmers want fast, cheap, and large memory, they'll need to trade off one of them. The capacity of various speeds determines the size of solvable problems. While the "cache" is a great workaround for memory limits, there are *always* hard tradeoffs between fast and large.
...
1. Issue analysis - identify the information that must be represented, and how the selected [programming language](cs-langs.md) will represent it.
...
7. Testing - use the examples as tests and make sure the function passes all of them, which helps find all [errors](cs-software-redesign.md).
...
3. Implementation: building the actual software.
   - This is where [coding](cs-langs.md) happens, including the [IDE](cs-software-ide.md) and [VCS](cs-software-versionctrl.md).
   - Incidentally, it's also the greatest value-added place for code-based [machine learning models](cs-ai-ml.md).
4. Testing: checking for [bugs, issues, and performance problems](cs-software-redesign.md).
...
To make less painful [errors](cs-software-redesign.md) (especially logic errors), good software development tests the code incrementally:
