...
A scanner ends up producing the same abstraction as a [camera](engineering-camera.md): a recorded image. However, the primary difference between the two is that a scanner will work line-by-line while a camera captures the image as instantaneously as possible.
