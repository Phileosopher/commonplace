...
On the granular level, software development is about reliable [version control](/version-control/), but good development requires [*constant* direct feedback](https://notageni.us/cust-svc/) from the users. This is harder than it sounds because the users often don't know what they want.
...
- It's not uncommon for many of them to have unspecified [PTSD](https://gainedin.site/ptsd/) about the *last* effort they took to simply get the software to work correctly in the first place.
...
- They don't have the [bias](https://gainedin.site/bias/) of having worked on it, meaning they're more fine with destroying and rebuilding huge portions of the codebase.
- Their ability to get their mind around the [algorithms](/algorithms/) and [flow of data](/data-structures/) is a new experience to them altogether (and therefore enjoyable).
...
In software, [the UI](/ux-ui/) can hide away most features, so there are nowhere *near* as many constraints to adding features as most other engineering. This is good because it allows the core function of the software to be relatively unaffected, and bad because the list of features can be theoretically endless.

The more time software developers work on features, the less they're working on the core function of the software.

One of the most significant sources of slowdown and [cybersecurity risks](/security/) comes from using an off-the-shelf [framework](/prog-features/) for an application:

- That framework will likely be sufficient (even to easily [scale](/enterprise/)), but over time the developers of *that* framework can often add arbitrary features that will severely slow it down (and therefore the dependent project).
- Even without a framework, a sufficiently complex framework-free application will have an ad-hoc, [informally-specified](/documentation/), [bug-ridden](/software-redesign/), slow implementation of half of a framework.
...
Over time, a software can develop a strong "user base", which means the software developers will have a clearer interest in developing the software further. This usually creates a [marketing](https://notageni.us/marketing/) incentive to improve user retention by adding more features, even when *very* few users requested that feature.

However, more features create more avenues of maintenance. The [nature of specialization](https://gainedin.site/specialization-cities/) means that the software will slowly advance to several possible states:

1. The software is finished. Outside of [security patches](/security/) or [OS updates](/os/), there is no further need to work on the software.
...
- Some do precisely the same thing, but are [open-source](/floss/) or disable telemetry.
- There will be an entirely new implementation that carries the spirit of the original software.

Even then, [software trends](/trends/) move so fast that it doesn't really matter. If the developer made money on it before it became obsolete, it was good enough.

## End-of-life

Often, a software will become "deprecated":

- If the lead developer [passes on](https://gainedin.site/legacy/), but [with a succession plan](https://adequate.life/death/), nothing in the software itself will change, but the brilliant mind that [created that software](/software-design/) will be difficult to replace.
...
- If a superior software came along that was designed more efficiently or with more features, the developer may release it as [open-source](/floss/). From there, other developers will likely borrow parts of the code for *their* future projects.

Sometimes, however, a [large company](/enterprise/) will buy out well-maintained software. From that point, the software will likely suffer one of several fates:

- In the pursuit of profit, the software will be modified to [track more data](/faang/) or will develop a [freemium business model](/floss/).
- If the software was [open-source](/floss/), the developer or company will release a closed-source version of it, and another developer will subsequently [fork the software](/version-control/) as another open-source implementation.
