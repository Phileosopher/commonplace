...
Computer commands *could* distill down as [binary information](logic-cs.md) (and, in practice, you always *can* examine them inside [memory registers](cs-memory.md)) but that's absolutely tedious and difficult when you want to actually do something that *isn't* computer science.
...
- Instructions for how the [CPU](cs-cpu.md) will work with various inputs or outputs (also called a "driver").
- Instructions for routine things the [operating system](cs-os.md) must do frequently (like "refresh" the screen or check for new USB cables, also called a "process").
...
One unique type of file type is a directory. A directory is essentially a link to *all* the files contained in that directory. Then, that directory can have subdirectories doing the same thing. The entire thing assembles into a [tree data structure](data-structures.md), but it's essentially a lot of files that indicate other files' presence.

The key illusion is that the files appear to be the free expression of a specific implementation, but at the bottom they are simply more [code](cs-langs-features.md).
...
When you first [boot](cs-boot.md) any computer, a few files are already running:

- Input file (stdin) - tracks what you're doing with the [keyboard](cs-keyboard.md) (and occasionally the [mouse](cs-mouse.md)).
- Output file (stdout) - feeds information to the [screen](engineering-screen.md)/[printer](engineering-printers.md)/[speakers](engineering-audio.md).
- Error file (stderr) - a database of [errors](cs-software-redesign.md) with reference codes that specify how things broke.
...
- File [permissions](cs-infosec.md), as well as the file's owner, and "access control lists/entries" that indicate which users can do what (i.e., write/read/rename/delete)
...
Files can also contain all sorts of multimedia, and it can get a bit complicated. Most codecs for [music files](engineering-audio.md), for example, don't usually contain *only* music. An MP3, for example, will have plenty of text data about the performer, [licensing/copyright](legal-ip.md) information, song lyrics, and album photos. For [memory allocation](cs-langs.md) reasons of all the *possible* metadata, this can *really* pad out file sizes.

Further, files can be archives (where they contain one or more files together) or compressed (where an [algorithm](cs-langs-algorithms.md) finds and replaces patterns of text). Some standards are compressed archives (e.g., zip), some are archives alone, (e.g., tar), and some can compress files that aren't designated archives (e.g., gz).

It's worth noting that there are often specific rules about what characters work in file names. All the alphabet is allowed, along with some special characters like commas, periods, and spaces. However, [regular expression](cs-langs.md) elements like "?" and "/" aren't allowed.
...
Since [cloud storage](cs-distsys-cloud.md) became popular, some files can be labeled as a network resource instead of as a local one. While this can be profoundly convenient, it can confuse users who don't realize their files are *not* stored locally.
...
- At one time, software such as Disk Defragmenter on [Windows](cs-os-windows.md) would defragment.
- [Linux](cs-os-unix.md) computers sidestepped this issue by placing the files at comparatively large distances from each other.
- Now, many [algorithms](cs-langs-algorithms.md) can predict fragmentation before it happens and accommodate it by either moving around blocks or writing to a different part of the disk.
...
- Deduplication - getting rid of duplicates, which can save money in [enterprise systems](cs-distsys-enterprise.md)
...
- Trim support - in [solid-state drives](cs-memory.md), writes all the unused data to 0 to prolong the life of the drive

Because of the way [algorithms](cs-langs-algorithms.md) write blocks, you can save a *lot* of disk space if you group similar information together *before* compressing it for archiving. This can heavily affect [compiling sizes](cs-langs.md) as well.
...
File systems exist within various [standards](cs-standards.md), most of them based on the type of [operating system](cs-os.md) the computer uses. Some older file systems (e.g., FAT, FAT32) had a bad tendency to randomly lose data, and there are various file system features meant to make files safer and faster:
...
- Journaling - keeping a record file in long-term memory of tasks the CPU still has to do, very useful if the task is interrupted and can't continue, can sometimes *only* journal the metadata, a critical part of [cybersecurity](cs-infosec.md).
- File change log - keeps a record of many changes to various files, can be localized to system files or apply to *all* files.
- Case-sensitivity - Some operating systems (like [Unix-based ones](cs-os-unix.md)) are case-sensitive ("file.txt" *isn't* "File.txt"), while others (like [Windows](cs-os-windows.md)) are not ("file.txt" is the same as "FiLe.TXt"). Occasionally, a case-insensitive file system will still *preserve* the case-sensitivity.
...
Logical file systems fit squarely in the domain of [good UX](engineering-design.md), and getting rid of them to allow text-based entry and output is why [CLI](cs-cli.md) is still fashionable for tech-savvy users.
...
There are various hierarchical file systems (exFat, ext2/3/4, jfs, gpfs etc.) which are each designed with different purposes in mind. The file system rules (and where to start the operating system's first programs) are stored in the [boot sector](cs-boot.md).

Another convenience of a hierarchical file system is that "moving" a file on the same disk is simply renaming the file path, which may mean changing a few [memory references](cs-memory.md) depending on the design of the hardware. This is why a drag-and-drop transfer can sometimes be instant. This, however, creates a [UX hangup](engineering-design.md) when moving across disks, since lots of data will take a while to transfer.

It's worth being aware of what a file is doing when you're copying, moving, and deleting:

- When you delete a file, the file is still there, but it gets marked for overwriting whenever the operating system needs more storage. Unless you actively use deletion software, [it'll still be there](cs-infosec-compliance.md) if the operating system never overwrites it.
...
- Drag-and-drop with the mouse could mean *either* moving or copying depending on the [operating system](cs-os.md) and where it's going, so it's better to use the cut or copy keyboard shortcut and then the paste shortcut.

However, there are some limits to hierarchical files based on how many bits were allocated to the task. At one point, [memory](cs-memory.md) limits had a hard 256-character limit on the file. While this sounds like a lot, that character limit included the *folders* as part of that file name.
...
- This gets worse if you're dealing with [cloud storage](cs-distsys-cloud.md) and [distributed systems](cs-distsys.md), and *especially* with [virtual machines](cs-distsys-vm.md).
...
Beyond the [boot sector](cs-boot.md), drives have "partitions" assigned for various purposes:
...
Except for some forms of [virtualization](cs-distsys-vm.md), it's only sane to keep up to 1 operating system on 1 partition "volume". This evenly divides the tasks of an operating system in such a way that there's no conflict for what the CPU should do.

Volume management in [large-scale systems](cs-distsys.md) can become challenging. It can help to use a native logical volume management file system (e.g., LVM-ext4, Btrfs), or it can be done higher up in the [OS stack](cs-os.md) via [virtualization](cs-distsys-vm.md).

Partitions are grouped based on the operating system.

- In [Windows-based](cs-os-windows.md) systems, partitions are given arbitrary letters (C: as the first one because A: and B: were once for floppy disks, D:, E:, etc.).
- In [Unix-based](cs-os-unix.md) systems, they're grouped under specific folders (and accessible with the "df -h" command).
