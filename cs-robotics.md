...
- It always requires an input with sensors that detect something that physically changes. This input is often a [camera](engineering-camera.md) or [microphone](engineering-audio.md), and often includes sensors along with it. If it's a simple-enough robot, it just needs sensors.
- The [operating system](cs-os.md) is relatively similar to any other computer, though interacting in the "real world" is *absurdly* complex compared to working strictly with raw information.
- The [processor](cs-cpu.md) eventually operates an output that physically does something. It may use a [screen](engineering-screen.md), but almost always uses a mechanical arm, and typically has lots of driver "middleware" to accommodate the extra mechanical hardware.
...
The first industrial robot was Unimate, created in the 1950s by George Devol. It worked on an assembly line at a New Jersey General Motors plant and worked off drum [memory](cs-memory.md).
...
However, after a few trial runs programming a brick-laying robot, you'll notice it creating bad brickwork. [Debugging](cs-software-redesign.md) would show details necessary for a successful task:
...
2. Set an [algorithm](cs-langs-algorithms.md) that clarifies exactly what a pebble is, then create [instructions](cs-langs.md) to grab it, with each grabbing effort modulated with the camera data for every single pebble.
...
Robot [AI](cs-ai.md) must also incorporate elements of kinesiology to manage complex tasks.
...
Lately, [machine learning algorithms](cs-ai-ml.md) have dramatically improved the variability of the robot's instructions. They still have some trouble improvising while crossing difficult terrain, but can balance enough to ride skateboards or zip lines.
...
- The logic necessary to frame robotic instructions is conventional [software design](cs-software.md) work.
...
- Even after many iterations, the work to [revise a robot's code](cs-software-redesign.md) is never technically done.
...
- In the event of a poorly programmed robot, high-risk robotic implementations (e.g., [cars](cs-autos.md)) won't withstand human nature's tendency to [blame](people-rules.md) when people die, so it becomes a heavily [politicized](power.md) situation far more than simply upgraded [technology](technology.md).
...
- Like realistic [game development](cs-software-gamedev.md), [widespread adoption](people-trends.md) of robots will always be suppressed by the [uncanny valley](https://en.wikipedia.org/wiki/Uncanny_valley):
