...
The more information, the more time or [memory](cs-memory.md) the computer will need to work with it.
...
To send [files](cs-files.md) across a [network](cs-networks.md) or [store long-term](cs-memory.md) without using it, it makes sense to cut down on as much space as possible.
